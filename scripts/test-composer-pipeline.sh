...
# 재생성 E2E만: E2E_COMPOSER_REGENERATE=1 npm run test:composer-pipeline
# CI·자동 기동: npm run test:e2e:composer-pipeline:ci:all (재생성 E2E 포함)
set -euo pipefail
...
elif [ "${E2E_COMPOSER_REGENERATE:-}" = "1" ]; then
  echo ""
  echo "--- E2E (composer-regenerate) ---"
  npm run test:e2e:composer-regenerate
fi
