...
describe('uiPreferences', () => {
  const save = {
    ...
    projectsLegacy: process.env.REACT_APP_UI_PROJECTS_LEGACY,
    genspark: process.env.REACT_APP_UI_GENSPARK_PRIMARY,
  };

  afterEach(() => {
    process.env.REACT_APP_UI_PROJECTS_ENABLED = save.projects;
    process.env.REACT_APP_UI_PROJECTS_LEGACY = save.projectsLegacy;
  ...
  it('프로젝트 UI 켬이면 기본 젠스파이크 우선은 끔(명시 true 전까지)', () => {
    ...
    delete process.env.REACT_APP_UI_PROJECTS_LEGACY;
    delete process.env.REACT_APP_UI_GENSPARK_PRIMARY;
    expect(isUiProjectsEnabled()).toBe(true);
    expect(isUiProjectsLegacySurfaceEnabled()).toBe(false);
    expect(isGensparkPrimaryExperience()).toBe(false);
    expect(getStandaloneChatPath()).toBe('/');
  });

  it('LEGACY+ENABLED 일 때만 레거시 /projects 표면이 켜진다', () => {
    process.env.REACT_APP_UI_PROJECTS_ENABLED = 'true';
    process.env.REACT_APP_UI_PROJECTS_LEGACY = 'true';
    expect(isUiProjectsLegacySurfaceEnabled()).toBe(true);
  });
});
