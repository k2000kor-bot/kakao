...
import { FALLBACK_WS_ORIGIN } from '../../config/api';
...
describe('useWebSocket', () => {
    ...
    it('메시지를 수신해야 함', async () => {
        ...
        if (result.current.socket) {
            ...
            (result.current.socket as WebSocket & { onmessage: (e: MessageEvent) => void }).onmessage(messageEvent);
        }
    ...
    it('에러를 처리해야 함', async () => {
        ...
        if (result.current.socket) {
            ...
            (result.current.socket as WebSocket & { onerror: (e: Event) => void }).onerror(errorEvent);
        }
