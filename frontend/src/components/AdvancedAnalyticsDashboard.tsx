...
const AdvancedAnalyticsDashboard: React.FC = () => {
    ...
    const loadAnalyticsData = async () => {
        try {
            ...
            const [overviewRes, behaviorRes, aiRes, businessRes, predictionsRes] = await Promise.all([
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ANALYTICS_ROUTER_OVERVIEW_PATH)),
                axios.get(
                    joinApiHealthCheckUrl(
                        CONFIG_API_ORIGIN,
                        `${ANALYTICS_ROUTER_USER_BEHAVIOR_PATH}?${API_QUERY_PARAM_PERIOD}=30d`,
                    ),
                ),
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ANALYTICS_ROUTER_AI_PERFORMANCE_PATH)),
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ANALYTICS_ROUTER_BUSINESS_METRICS_PATH)),
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ANALYTICS_ROUTER_PREDICTIONS_PATH))
            ]);
    ...
    const handleGenerateCustomReport = async () => {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ANALYTICS_ROUTER_CUSTOM_REPORT_PATH), reportConfig);
    ...
    const handleExportReport = async (format: string) => {
        try {
            const response = await axios.get(
                joinApiHealthCheckUrl(
                    CONFIG_API_ORIGIN,
                    `${ANALYTICS_ROUTER_EXPORT_PATH_PREFIX}/${encodeURIComponent('RPT_123')}?${API_QUERY_PARAM_EXPORT_FORMAT}=${encodeURIComponent(format)}`,
                )
            );
            if (response.data.success) {
                // 다운로드 링크 생성
    ...
    const getStatusColor = (value: number, good: number, warning: number): 'success' | 'warning' | 'error' => {
                        ...
                        <CardContent>
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                                ...
                                <Chip
                                    ...
                                    color={getStatusColor(overview.error_rate, 1, 3)}
                                    size="small"
                                />
                            </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
                        ...
                        <TextField
                            ...
                            onChange={(e) => setReportConfig({
                                ...reportConfig, 
                                metrics: e.target.value.split(',').map((m) => coerceTrimmedString(m, '')).filter(Boolean)
                            })}
                        />
                    </Box>
                </DialogContent>
