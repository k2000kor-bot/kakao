...
import {
    API_SYSTEM_BACKUP_PATH,
    API_SYSTEM_METRICS_PATH,
    API_SYSTEM_RESTART_PATH,
...
function UltimateSystemInterface() {
    ...
    const updateSystemStatus = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), API_SYSTEM_STATUS_PATH)
            );
    ...
    const updateSystemMetrics = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), API_SYSTEM_METRICS_PATH)
            );
            const data = await response.json();
    ...
    // 알림 읽음 처리
    const _markNotificationAsRead = useCallback((notificationId: string) => {
    ...
    const restartSystem = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), API_SYSTEM_RESTART_PATH),
                {
                    method: 'POST',
                }
            );
    ...
    const backupSystem = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), API_SYSTEM_BACKUP_PATH),
                {
                    method: 'POST',
                }
            );
    ...
    // 시스템 상태 결정
    const getSystemStatusColor = useCallback((status: string): 'success' | 'warning' | 'error' | 'default' => {
        switch (status) {
                ...
                <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <Chip
                            ...
                            color={getSystemStatusColor(systemStatus.overall)}
                            size="medium"
                        />
                    </Box>
                    <LinearProgress
    ...
    return (
        <Box sx={{ display: 'flex', height: 'var(--app-vh)' }}>
            {/* 상단 앱바 */}
            <AppBar position="fixed" sx={{ zIndex: 'var(--z-app-bar)' }}>
                <Toolbar>
                    <IconButton
                        color="inherit"
                        edge="start"
