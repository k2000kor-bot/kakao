...
function AdvancedAIEngine() {
    ...
    const collectMetrics = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), AI_ENGINE_METRICS_PATH)
            );
    ...
    const updateModelStatus = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), AI_MODELS_STATUS_PATH)
            );
            const data = await response.json();
    ...
    const runProcessingPipeline = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), AI_PROCESS_PATH),
                {
                    method: 'POST',
    ...
    const retrainModel = useCallback(async (modelId: string) => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(
                    resolveApiBaseUrl(),
                    `${AI_MODELS_BASE_PATH}/${modelId}/retrain`
                ),
                {
                    method: 'POST',
                }
    ...
    const optimizeModel = useCallback(async (modelId: string) => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(
                    resolveApiBaseUrl(),
                    `${AI_MODELS_BASE_PATH}/${modelId}/optimize`
                ),
                {
                    method: 'POST',
                }
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid size={{ xs: 12, sm: 4 }}>
                            ...
                            <Button
                                ...
                                fullWidth
                                type="button"
                                onClick={() => void runProcessingPipeline()}
                                disabled={isProcessing || !coerceTrimmedString(inputText, '')}
