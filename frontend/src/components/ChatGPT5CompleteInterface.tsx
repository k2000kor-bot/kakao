...
interface ChatMessage {
    ...
    response?: string;
    timestamp: string;
    /** 생성 단계 임시 답변 행 — 별도 타이핑 카드와 중복 표시 방지 */
...
const AI_MODELS = [
    { id: 'codex', name: 'Codex', icon: QrCode, color: 'var(--accent-info)' },
    { id: 'sora', name: 'Sora', icon: PlayArrow, color: 'var(--accent-success)' },
    { id: 'gpt', name: 'GPT', icon: GridView, color: 'var(--accent-warning)' },
    { id: 'chat', name: '챗', icon: Chat, color: 'var(--accent-secondary)' },
];
...
export const ChatGPT5CompleteInterface: React.FC = () => {
    ...
    const [_showFileManager, _setShowFileManager] = useState(false);
    const [_showSessionList, setShowSessionList] = useState(false);
    ...
    // 고급 AI 기능 상태
    const [_qualityMode, _setQualityMode] = useState<'standard' | 'enhanced' | 'expert'>('enhanced');
    const [autoReview, _setAutoReview] = useState(true);
    const [_qualityThreshold, _setQualityThreshold] = useState(85);
    const [_qualityReviews, setQualityReviews] = useState<QualityReview[]>([]);

    // 딥러닝 및 머신러닝 시스템 상태
    const [deepLearningModels, setDeepLearningModels] = useState<DeepLearningModel[]>([]);
    const [activeTrainingSessions, _setActiveTrainingSessions] = useState<TrainingSession[]>([]);
    const [autoLearningConfig, _setAutoLearningConfig] = useState<AutoLearningConfig>({
        enabled: true,
        autoRetrain: true,
    ...
    const [_mlPerformanceMetrics, _setMLPerformanceMetrics] = useState<MLPerformanceMetrics | null>(null);
    const [_isModelTraining, _setIsModelTraining] = useState(false);
    ...
    const [_selectedModel, setSelectedModel] = useState<DeepLearningModel | null>(null);
    ...
    const [_collaborators, _setCollaborators] = useState<Collaborator[]>([
    ...
    const [_showSecurityDashboard, _setShowSecurityDashboard] = useState(false);
    const [_showPerformanceOptimizer, _setShowPerformanceOptimizer] = useState(false);
    const [_showSystemIntegration, _setShowSystemIntegration] = useState(false);
    const [_showAIIntelligence, _setShowAIIntelligence] = useState(false);
    ...
    const [_showPersistentSessionManager, setShowPersistentSessionManager] = useState(false);
    const [_sessionSearchQuery, _setSessionSearchQuery] = useState('');
    const [_sessionStats, setSessionStats] = useState<ChatSessionStats | null>(null);
    ...
    // 노트북 LLM 상태
    const [_showNotebookLLM, _setShowNotebookLLM] = useState(false);
    const [_showProjectNotebookLLM, _setShowProjectNotebookLLM] = useState(false);
    ...
    }, [autoLearningConfig.hyperparameterOptimization]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    }, [activeTrainingSessions]); // eslint-disable-line react-hooks/exhaustive-deps -- deepLearningModels
    ...
    const handleCreateProject = async () => {
        const trimmedName = coerceTrimmedString(projectName, '');
        ...
        try {
            ...
            errorLogger.error('프로젝트 생성 실패', error, { projectName: trimmedName });
            // 오프라인 모드로 로컬 생성
    ...
    const handleSendMessage = async (overrideText?: string) => {
        const trimmed = coerceTrimmedString(overrideText, inputValue);
        if (!trimmed || !currentSession) return;

        // 새로운 질문이 시작되면 지속적 대화 세션 생성
        if (!currentPersistentSession) {
            const sessionTitle = conversationListTitleFromUserMessage(trimmed);
            createNewPersistentSession(sessionTitle, trimmed);
        ...
        let clearComplete5NsPhases = scheduleAssistantNonStreamLoadingPhaseTimers((text) => {
            setMessages((prev) =>
        ...
        try {
            // 통합 AI 응답 생성 시스템 사용
            let aiResponse = await generateIntegratedAIResponse(trimmed);

            // 답변 품질 향상 적용
            if (autoReview) {
                aiResponse = await enhanceAnswerQuality(aiResponse, trimmed);
            }

            clearComplete5NsPhases();
            clearComplete5NsPhases = () => {};
            ...
            const assistantMessage: ChatMessage = {
                ...
                content: aiResponse,
                timestamp: new Date().toISOString(),
                qualityScore: evaluateAnswerQuality(aiResponse, trimmed).overallScore,
                reviewStatus: 'reviewed',
                feedback: [],
            };

            // 품질 검토 생성 및 저장
            const qualityReview = createQualityReview(assistantMessage.id, aiResponse, trimmed);
            setQualityReviews(prev => [...prev, qualityReview]);

            setMessages((prev) =>
                prev.map((m) => (m.id === placeholderAssistantId ? assistantMessage : m))
            );
            ...
            if (autoLearningConfig.enabled) {
                ...
                updateSystemLearning(trimmed, aiResponse, simulatedFeedback);
            }
            ...
            setMessages((prev) =>
                prev.map((m) => (m.id === placeholderAssistantId ? errorMessage : m))
            );
        } finally {
            clearComplete5NsPhases();
            clearComplete5NsPhases = () => {};
    ...
    const handleModifyRequestConfirm = async (modifyRequest: string) => {
        ...
        const modifyTargetId = selectedMessageForModify.id;
        setMessages((prev) =>
            prev.map((msg) =>
                msg.id === modifyTargetId
                    ? { ...msg, content: ASSISTANT_PLACEHOLDER_ANALYZING, generationPlaceholder: true }
                    : msg
            )
        );

        let clearModifyNsPhases = scheduleAssistantNonStreamLoadingPhaseTimers((text) => {
            setMessages((prev) =>
                prev.map((msg) => (msg.id === modifyTargetId ? { ...msg, content: text } : msg)),
            );
        });
        ...
        try {
            ...
            await runAssistantNonStreamPostResponsePhases((text) => {
                setMessages((prev) =>
                    prev.map((msg) =>
                        msg.id === modifyTargetId
                            ? { ...msg, content: text, generationPlaceholder: true }
                            : msg,
                    ),
                );
            });
            ...
            // 지속적 대화 세션에 수정된 메시지 추가 (기존 메시지는 로컬에서만 업데이트)
    ...
    const handleCreateNewSession = async () => {
        ...
        try {
            const projectService = ChatGPTProjectService.getInstance();
            const newSession = await projectService.createSession(currentProject.id, '새 대화');
    ...
    const _generateAIResponse = async (userInput: string): Promise<string> => {
        // 키워드 기반 응답 생성
...
어떤 기능에 대해 더 자세히 알고 싶으시거나, 특정 작업을 수행하고 싶으시면 말씀해 주세요!`;
    ...
    const handleQuickStart = (type: string) => {
        ...
        const text = messages[type as keyof typeof messages];
        setInputValue(text);
    ...
    const createNewPersistentSession = async (title: string, initialMessage?: string) => {
        ...
        errorLogger.info('새로운 지속적 대화 세션 생성', { sessionTitle: newSession.title });
    };

    const _switchToPersistentSession = (sessionId: string) => {
        ...
        if (session) {
            ...
            errorLogger.info('지속적 대화 세션으로 전환', { sessionTitle: session.title });
        }
    };
    ...
    const _deletePersistentSession = async (sessionId: string) => {
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps -- cleanupInactiveSessions is stable
    }, []);

    return (
        <Box sx={{ display: 'flex', height: 'var(--app-vh)', bgcolor: 'var(--bg-secondary)' }}>
            {/* 왼쪽 사이드바 (데스크톱만 표시) */}
            <Box sx={{
                width: 280,
                bgcolor: 'white',
                ...
                <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                        <Box sx={{ width: 24, height: 24, bgcolor: 'var(--accent-info)', borderRadius: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                ...
                                <Tooltip key={model.id} title={model.name}>
                                    <IconButton
                                        ...
                                        sx={{
                                            ...
                                            '&:hover': {
                                                bgcolor: isSelected ? model.color : 'var(--bg-tertiary)'
                                            }
                                        }}
                                    >
                                        <Icon />
                ...
                <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                        ...
                        <IconButton type="button" size="small" onClick={() => setShowProjectCreation(true)} aria-label="새 프로젝트 만들기">
                            <Add />
                        </IconButton>
                    </Box>
                                ...
                                >
                                    <CardContent sx={{ p: 1.5 }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            {project.name === '한양2차' ? (
                                                <Diamond sx={{ color: 'var(--accent-secondary)', fontSize: 20 }} />
                                            ) : (
                                                <Folder sx={{ color: 'var(--accent-info)', fontSize: 20 }} />
                                            )}
                                                ...
                                                {projectSessions.map((session) => (
                                                    <Box
                                                        ...
                                                        sx={{
                                                            ...
                                                            borderRadius: 1,
                                                            cursor: 'pointer',
                                                            bgcolor: currentSession?.id === session.id ? 'var(--bg-secondary)' : 'transparent',
                ...
                <Box sx={{ p: 2, borderTop: 1, borderColor: 'divider' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Avatar sx={{ width: 32, height: 32, bgcolor: 'var(--accent-info)' }}>
                            <Person />
                        </Avatar>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            {currentProject.name === '한양2차' ? (
                                                <Diamond sx={{ color: 'var(--accent-secondary)', fontSize: 24 }} />
                                            ) : (
                                                <Folder sx={{ color: 'var(--accent-info)', fontSize: 24 }} />
                                            )}
                                    ...
                                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                        ...
                                        <Box sx={{ borderTop: 1, borderColor: 'divider', bgcolor: 'white', p: 2 }}>
                                            <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1 }}>
                                                <IconButton type="button" size="small" aria-label="파일 또는 콘텐츠 첨부">
                                                    <Plus />
                                                </IconButton>
                                                ...
                                                <TextField
                                                    ...
                                                    placeholder={`${currentProject.name}에서 새 대화`}
                                                    inputProps={{ 'aria-label': '대화 메시지 입력' }}
                                                    variant="outlined"
                                                    size="small"
                                                    onKeyDown={(e) => {
                                                ...
                                                <IconButton type="button" size="small" aria-label="음성 출력">
                                                    <VolumeUp />
                                                </IconButton>
                                            </Box>
                                        </Box>
                                    ...
                                    <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <Box sx={{ textAlign: 'center' }}>
                                            ...
                                            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                                                새 대화 버튼을 클릭하여 대화를 시작하거나 기존 세션을 선택하세요
                                            </Typography>
                                            <Button
                                                type="button"
                                                variant="contained"
                                                startIcon={<Add />}
                                                onClick={handleCreateNewSession}
                                                sx={{ bgcolor: 'var(--accent-info)', '&:hover': { opacity: 0.9 } }}
                                                aria-label="새 대화 시작"
                                            >
                                                새 대화 시작
                                            </Button>
            ...
            <Dialog open={showProjectCreation} onClose={() => setShowProjectCreation(false)} maxWidth="md" fullWidth>
                <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Box sx={{ ml: 'auto', display: 'flex', gap: 1 }}>
                        <Button
                            ...
                            aria-label="프로젝트 템플릿 선택"
                        >
                            템플릿 선택
                        </Button>
                        <IconButton type="button" size="small" onClick={() => setShowMemorySettings(!showMemorySettings)} aria-label={showMemorySettings ? '메모리 설정 닫기' : '메모리 설정'}>
                            <Settings />
                        </IconButton>
                        <IconButton type="button" size="small" onClick={() => setShowProjectCreation(false)} aria-label="프로젝트 생성 모달 닫기">
                            <MoreVert />
                        </IconButton>
                    </Box>
                </DialogTitle>
                ...
                <DialogActions>
                    <Button type="button" onClick={() => setShowProjectCreation(false)} aria-label="프로젝트 생성 취소">
                        취소
                    </Button>
                    <Button
                        type="button"
                        ...
                        disabled={!coerceTrimmedString(projectName, '')}
