...
import { PWA_INSTALL_DISMISSED_AT_STORAGE_KEY } from '../services/pwaInstallStorageKeys';
...
const PWAInstallPrompt: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isInstallable && !isInstalled && canInstall) {
      ...
      if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
        const dismissedInstall = globalThis.localStorage.getItem(PWA_INSTALL_DISMISSED_AT_STORAGE_KEY);
  ...
  const handleDismiss = useCallback(() => {
    setShowPrompt(false);
    if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
      globalThis.localStorage.setItem(PWA_INSTALL_DISMISSED_AT_STORAGE_KEY, new Date().toISOString());
    }
  }, []);
  ...
  const _handleCheckUpdates = useCallback(async () => {
    try {
    ...
    <>
      ...
      >
        <Card sx={{ 
          minWidth: 320, 
          maxWidth: 400,
          background: 'linear-gradient(135deg, var(--accent-info-figma, var(--accent-info)) 0%, var(--accent-secondary) 100%)',
          color: 'var(--on-accent)'
        }}>
          <CardContent sx={{ pb: 2 }}>
                ...
                {getInstallBenefits.slice(0, 3).map((benefit, index) => (
                  <Box key={`benefit-${index}-${benefit.substring(0, 5)}`} component="li">
                    <Chip
                      ...
                      size="small"
                      sx={{ 
                        backgroundColor: 'var(--surface-overlay, rgba(255,255,255,0.2))',
                        color: 'white',
                        fontSize: '0.75rem'
                      }}
                    />
            ...
            <Box component="fieldset" sx={{ display: 'flex', gap: 1, border: 'none', p: 0, m: 0 }} aria-label="설치 액션">
              <Button
                ...
                sx={{
                  ...
                  color: 'white',
                  '&:hover': {
                    backgroundColor: 'var(--surface-overlay-hover, rgba(255,255,255,0.3))'
                  }
                }}
              >
              ...
              <Button
                ...
                sx={{
                  color: 'white',
                  borderColor: 'var(--border-overlay, rgba(255,255,255,0.3))',
                  '&:hover': {
                    borderColor: 'var(--border-overlay-hover, rgba(255,255,255,0.5))'
                  }
                }}
              >
                나중에
