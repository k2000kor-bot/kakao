...
} from '../services/writingUiStorageKeys';
...
const SearchPanel: React.FC<SearchPanelProps> = ({ isOpen, onClose, onSelect, onSearchTermChange }) => {
  ...
  const searchTrimmed = coerceTrimmedString(searchTerm, '');
    ...
    try {
      ...
      if (filters.writings) {
        if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
          const writings = JSON.parse(globalThis.localStorage.getItem(WRITING_HISTORY_STORAGE_KEY) || '[]');
        writings.forEach((writing: Record<string, unknown>) => {
          const template = String(writing.template ?? '');
          ...
          if (
            ...
            searchResults.push({
              id: (writing.id as string) || `writing-${Date.now()}`,
              type: 'writing',
              title: template || '글쓰기',
              content: content || '',
              timestamp: (writing.createdAt as string) || new Date().toISOString(),
              metadata: { template, category: writing.category },
            });
          }
        });
        }
      ...
      if (filters.templates) {
        if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
          const templates = JSON.parse(globalThis.localStorage.getItem(WRITING_TEMPLATES_STORAGE_KEY) || '[]');
        templates.forEach((template: Record<string, unknown>) => {
          ...
          if (
            ...
            searchResults.push({
              id: (template.id as string) || `template-${Date.now()}`,
              type: 'template',
              title: title || '템플릿',
              content: description || '',
      ...
      if (filters.files) {
        if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
          const files = JSON.parse(globalThis.localStorage.getItem(UPLOADED_FILES_STORAGE_KEY) || '[]');
        files.forEach((file: Record<string, unknown>) => {
          const name = String(file.name ?? '');
          if (name.toLowerCase().includes(term.toLowerCase())) {
            searchResults.push({
              id: (file.id as string) || `file-${Date.now()}`,
              type: 'file',
              title: name || '파일',
              content: String(file.description ?? ''),
              timestamp: (file.uploadedAt as string) || new Date().toISOString(),
              metadata: { fileName: name, fileType: file.type },
            });
          }
        });
        }
  ...
  useEffect(() => {
    if (searchTrimmed.length > 0) {
      performSearch(searchTrimmed);
      onSearchTermChange?.(searchTrimmed);
    } else {
      setResults([]);
      onSearchTermChange?.('');
    }
  }, [searchTrimmed, filters, onSearchTermChange, performSearch]);
  ...
  useEffect(() => {
    ...
    // handleSelectResult, onClose 의존성 의도적 제외 (키다운 시 선택만 반영)
    ...
    <div className="search-panel-overlay" onClick={onClose}>
      <div className="search-panel" onClick={(e) => e.stopPropagation()}>
        <div className="search-header">
          <div className="search-input-wrapper">
            <input
              ...
              aria-label="통합 검색 (메시지, 글쓰기)"
            />
            {isSearching && <div className="search-spinner">⏳</div>}
          </div>
          <button type="button" className="search-close-btn" onClick={onClose} aria-label="검색 패널 닫기">
            ✕
          </button>
        </div>
        ...
        <div className="search-results" ref={resultsRef}>
          {results.length === 0 && searchTrimmed.length > 0 && !isSearching && (
            <div className="search-empty">
              <p>검색 결과가 없습니다.</p>
            </div>
          )}
          {results.length === 0 && searchTrimmed.length === 0 && (
            <div className="search-empty">
              <p>검색어를 입력하세요.</p>
              ...
              <div className="result-content">
                ...
                {result.metadata && (
                  <div className="result-metadata">
                    {result.metadata.category != null && result.metadata.category !== '' && (
                      <span className="result-tag">{String(result.metadata.category)}</span>
                    )}
                    {result.metadata.fileType != null && result.metadata.fileType !== '' && (
                      <span className="result-tag">{String(result.metadata.fileType)}</span>
                    )}
                  </div>
                )}
              </div>
