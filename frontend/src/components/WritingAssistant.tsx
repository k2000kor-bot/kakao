...
import { API_V7_CHAT_PATH, joinApiHealthCheckUrl, resolveApiBaseUrl } from '../config/api';
...
const WritingAssistant: React.FC<WritingAssistantProps> = ({ onGenerate }) => {
  ...
  useEffect(() => {
    if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
      const stored = globalThis.localStorage.getItem(WRITING_TEMPLATE_FAVORITES_STORAGE_KEY);
      if (stored) {
  ...
  const toggleFavorite = useCallback((templateId: string) => {
    setFavorites((prev) => {
      const updated = prev.includes(templateId)
        ...
        : [...prev, templateId];
      if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
        globalThis.localStorage.setItem(WRITING_TEMPLATE_FAVORITES_STORAGE_KEY, JSON.stringify(updated));
      }
      return updated;
    });
  }, []);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const missingFields = requiredFields.filter(
      (f) => !coerceTrimmedString(formValues[f.name] ?? '', '')
    );
    ...
    try {
      ...
      const customType = coerceTrimmedString(customWritingType, '');
      if (customType) {
        finalFormValues.writingType = customType;
      }
      ...
      // 백엔드 API 호출
      const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), API_V7_CHAT_PATH), {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // 로컬 스토리지에 저장
        if (typeof globalThis !== 'undefined' && 'localStorage' in globalThis) {
          const history = JSON.parse(globalThis.localStorage.getItem(WRITING_HISTORY_STORAGE_KEY) || '[]');
          history.unshift({
          ...
          globalThis.localStorage.setItem(WRITING_HISTORY_STORAGE_KEY, JSON.stringify(history));
        }
      } else {
