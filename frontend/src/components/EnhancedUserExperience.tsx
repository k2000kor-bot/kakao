...
} from '../services/userPreferencesUiStorageKeys';
...
function EnhancedUserExperience() {
    ...
    const [activities, _setActivities] = useState<UserActivity[]>([
        {
            id: '1',
            type: 'achievement',
            title: 'AI 마스터 달성',
    ...
    const savePreferences = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), USER_PREFERENCES_PATH),
                {
                    method: 'POST',
            ...
            if (data.success) {
                // 로컬 스토리지에도 저장
                localStorage.setItem(USER_PREFERENCES_JSON_STORAGE_KEY, JSON.stringify(preferences));
            }
        } catch (error: unknown) {
    ...
    const updateUserStats = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), USER_STATS_PATH)
            );
            const data = await response.json();
    ...
    const submitFeedback = useCallback(async () => {
        try {
            const response = await fetch(
                joinApiHealthCheckUrl(resolveApiBaseUrl(), USER_FEEDBACK_PATH),
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    useEffect(() => {
        // 로컬 스토리지에서 선호도 로드
        const savedPreferences = localStorage.getItem(USER_PREFERENCES_JSON_STORAGE_KEY);
        if (savedPreferences) {
            ...
            <List>
                {activities.map((activity) => (
                    <ListItem key={activity.id}>
                        ...
                        <Chip
                            label={activity.type}
                            color={activity.color as 'default' | 'primary' | 'secondary' | 'error' | 'info' | 'success' | 'warning'}
                            size="small"
                        />
                    </ListItem>
                ))}
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid size={{ xs: 12 }}>
                            <Button
                                ...
                                disabled={!feedback.rating || !coerceTrimmedString(feedback.comment, '')}
                                startIcon={<Lightbulb />}
                            >
                                피드백 제출
                            </Button>
