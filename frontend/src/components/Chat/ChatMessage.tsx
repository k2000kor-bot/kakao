...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  const handleEditSave = () => {
    if (coerceTrimmedString(editingContent, '')) {
  ...
  const { ref: lazyRef, isLoaded: _isLoaded } = useLazyLoading({
    threshold: 0.1,
    rootMargin: '100px',
  ...
  const highlightText = useCallback((text: string, searchTerm?: string) => {
    ...
    return parts.map((part, index) => {
      ...
      return testRegex.test(part) ? (
        <mark key={`${part}-${index}`} style={{ backgroundColor: 'var(--accent-warning-muted)', padding: '0 2px' }}>
          {part}
        </mark>
      ) : (
  ...
  return (
    <article
      ...
      data-message-id={id}
      data-testid={highlighted ? 'message-highlighted' : undefined}
