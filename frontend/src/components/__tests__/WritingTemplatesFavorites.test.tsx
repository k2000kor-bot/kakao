...
describe('WritingTemplatesFavorites', () => {
  ...
  describe('렌더링', () => {
    ...
    it('localStorage에 즐겨찾기가 있으면 목록을 표시해야 함', () => {
      window.localStorage.setItem('writingTemplateFavorites', JSON.stringify(['template-1', 'template-2']));
  ...
  describe('즐겨찾기 토글', () => {
    it('즐겨찾기 버튼 클릭 시 즐겨찾기를 추가해야 함', () => {
      ...
      window.localStorage.setItem('writingTemplateFavorites', JSON.stringify(['template-1']));
      
      // 컴포넌트를 다시 렌더링
      const { rerender: _rerender } = render(<WritingTemplatesFavorites onSelectTemplate={mockOnSelectTemplate} />);
      ...
      if (favoriteButtons.length > 0) {
        ...
        const updated = JSON.parse(window.localStorage.getItem('writingTemplateFavorites') || '[]');
