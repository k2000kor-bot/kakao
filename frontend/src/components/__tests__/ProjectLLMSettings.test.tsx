/* eslint-disable jest/no-conditional-expect, testing-library/no-wait-for-side-effects, testing-library/no-wait-for-multiple-assertions */
...
const mockLocalLLMService: jest.Mocked<typeof localLLMService> = jest.mocked(localLLMService);
...
describe('ProjectLLMSettings', () => {
  ...
  beforeEach(() => {
    ...
    mockLocalLLMService.getProjectLLM.mockReturnValue(undefined);
  ...
  describe('렌더링', () => {
    ...
    it('닫기 버튼을 표시해야 함', () => {
      ...
      const closeButton = screen.getByLabelText('설정 모달 닫기');
  ...
  describe('로딩 상태', () => {
    it('모델 로딩 중일 때 로딩 메시지를 표시해야 함', async () => {
      ...
      mockLocalLLMService.getOllamaModels.mockReturnValue(promise as Promise<unknown[]>);
