...
import type { RetryResult, RetryOptions } from '../../utils/retryHandler';
...
const mockRetryWithNetworkCheck = jest.fn(
  async <T,>(fn: () => Promise<T>, _options?: RetryOptions): Promise<RetryResult<T>> => {
    try {
...
jest.mock('../../utils/retryHandler', () => {
  ...
  return {
    ...
    retryWithNetworkCheck: <T,>(fn: () => Promise<T>, options?: RetryOptions) => 
      mockRetryWithNetworkCheck(fn, options),
  };
});
...
describe('ErrorRecovery', () => {
  ...
  const _mockOnRecoveryFailure = jest.fn();

  beforeEach(() => {
    jest.clearAllMocks();
    mockGetOnlineStatus.mockReturnValue(true);
    mockSubscribe.mockReturnValue(jest.fn());
    mockRetryWithNetworkCheck.mockImplementation(async (fn: () => Promise<unknown>) => {
      try {
        const result = await fn();
        return { success: true as const, data: result, attempts: 1 };
      } catch (error) {
