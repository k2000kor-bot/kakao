/* eslint-disable jest/no-conditional-expect, testing-library/no-node-access, testing-library/no-wait-for-multiple-assertions, testing-library/no-wait-for-side-effects */
...
const mockMolitService: jest.Mocked<typeof molitRealEstateService> = jest.mocked(molitRealEstateService);
const mockRegistryService: jest.Mocked<typeof realEstateRegistryService> = jest.mocked(realEstateRegistryService);
...
describe('RealEstateDataPanel', () => {
  ...
  const mockStatistics: Record<string, unknown> = {
    transactionCount: 10,
  ...
  const mockRegistryChange: Record<string, unknown> = {
    id: '1',
    changeType: '소유권이전',
  ...
  const mockRegistryStats: Record<string, unknown> = {
    totalChanges: 5,
    ownershipChanges: 3,
    mortgageChanges: 1,
    leaseChanges: 1
  ...
  describe('실거래 정보 검색', () => {
    ...
    it('검색 실패 시 에러 메시지를 표시해야 함', async () => {
      ...
      fireEvent.click(searchButton);
      await waitFor(() => {
        expect(mockShowToast).toHaveBeenCalledWith('실거래 정보를 불러오는데 실패했습니다.');
      });
    });
  });
  ...
  describe('등기 변경 정보 검색', () => {
    ...
    it('등기 변경 정보 검색 실패 시 에러 메시지를 표시해야 함', async () => {
      ...
      fireEvent.click(screen.getByText('등기 변경 정보'));
      const searchButton = await screen.findByText('조회');
      fireEvent.click(searchButton);
      await waitFor(() => {
        expect(mockShowToast).toHaveBeenCalledWith('등기 변경 정보를 불러오는데 실패했습니다.');
      });
    });
  });
  ...
  describe('로딩 상태', () => {
    it('검색 중일 때 로딩 상태를 표시해야 함', async () => {
      ...
      mockMolitService.searchTransactions = jest.fn().mockReturnValue(promise as Promise<unknown[]>);
  ...
  describe('검색 파라미터', () => {
    ...
    it('거래유형을 선택할 수 있어야 함', () => {
      ...
      const transactionTypeSelect = screen.getByRole('combobox', { name: '거래유형' });
