 ...
 * 글쓰기 어시스턴트 기능 확인
 */
/* eslint-disable jest/no-conditional-expect, testing-library/no-node-access, testing-library/no-container, testing-library/await-async-query, testing-library/no-render-in-setup */
...
jest.mock('../WritingHistory', () => {
  return function MockWritingHistory({ onSelect }: { onSelect?: (item: { content: string; id: string }) => void }) {
    return (
...
jest.mock('../WritingEditor', () => {
  return function MockWritingEditor({ content, onSave, onImprove }: { content: string; onSave?: (v: string) => void; onImprove?: (type: string) => void }) {
    return (
...
jest.mock('../WritingQualityPanel', () => {
  return function MockWritingQualityPanel({ onImprove }: { content?: string; onImprove?: (s: string) => void }) {
    return (
...
jest.mock('../WritingStatisticsDashboard', () => {
  return function MockWritingStatisticsDashboard({ content }: { content?: string }) {
    return <div data-testid="writing-statistics-dashboard">통계: {content?.length ?? 0}자</div>;
  };
});

jest.mock('../WritingTemplatePreview', () => {
  const React = require('react');
  return function MockWritingTemplatePreview({ template, onSelect, onClose }: { template?: { title?: string }; onSelect?: () => void; onClose?: () => void }) {
...
jest.mock('../WritingAISuggestions', () => {
  return function MockWritingAISuggestions({ onApply }: { content?: string; onApply?: (s: { suggestion: string }) => void }) {
    return (
...
jest.mock('../../services/writingTemplates', () => {
  ...
  return {
    ...
    generatePrompt: (template: { title: string }, values: Record<string, unknown>, _tone?: string, _style?: string) => {
...
describe('WritingAssistant', () => {
  ...
  describe('즐겨찾기 기능', () => {
    beforeEach(() => {
      window.localStorage.setItem('writingTemplateFavorites', JSON.stringify(['test-template-1']));
    });
    ...
    it('즐겨찾기 버튼 클릭 시 즐겨찾기가 토글되어야 함', () => {
      ...
      // 즐겨찾기 상태가 변경되었는지 확인
      expect(window.localStorage.getItem('writingTemplateFavorites')).toBeTruthy();
    });
  ...
  describe('템플릿 미리보기', () => {
    it('미리보기 버튼 클릭 시 미리보기 모달이 표시되어야 함', async () => {
      ...
      if (previewButtons.length > 0) {
        ...
        previewButton = Array.from(allButtons).find((btn: Element) =>
  ...
  describe('폼 입력', () => {
    ...
    it('textarea 필드 입력이 작동해야 함', () => {
      ...
      const descriptionInput = screen.queryByLabelText(/설명/) ||
        ...
        screen.getAllByRole('textbox').find((el: HTMLElement) => el.tagName === 'TEXTAREA');
      if (descriptionInput) {
    ...
    it('어투 선택이 작동해야 함', () => {
      ...
      // select 찾기
      const toneSelects = screen.getAllByRole('combobox');
      const toneSelect = toneSelects.find((el: HTMLElement) =>
    ...
    it('스타일 선택이 작동해야 함', () => {
      ...
      const styleSelect = styleSelects.find((el: HTMLElement) =>
  ...
  describe('글 생성', () => {
    it('필수 필드가 없으면 생성 버튼 클릭 시 알림이 표시되어야 함', async () => {
      ...
      await waitFor(() => {
        expect(mockShowToast).toHaveBeenCalledWith(expect.stringContaining('다음 필드를 입력해주세요'));
      }, { timeout: 2000 });
    });
  ...
  describe('히스토리 기능', () => {
    beforeEach(() => {
      ...
      window.localStorage.setItem('writingHistory', JSON.stringify(history));
    });
  ...
  describe('생성된 글 관리', () => {
    ...
    it('복사 버튼 클릭 시 클립보드에 복사되어야 함', async () => {
      ...
      await waitFor(() => {
        expect(mockShowToast).toHaveBeenCalledWith('클립보드에 복사되었습니다!', 'success');
      }, { timeout: 3000 });
    });
