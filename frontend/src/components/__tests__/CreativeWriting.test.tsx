...
describe('CreativeWriting', () => {
  ...
  describe('스토리 생성', () => {
    ...
    it('장르 선택이 작동해야 함', async () => {
      ...
      const genreSelect = genreSelects.find((el: HTMLElement) =>
    ...
    it('스토리 생성 실패 시 에러 메시지가 표시되어야 함', async () => {
      ...
      const buttons = screen.getAllByRole('button');
      const generateButton = buttons.find(btn => btn.textContent?.includes('스토리 생성') && !(btn as HTMLButtonElement).disabled);
  ...
  describe('생성된 콘텐츠 관리', () => {
    ...
    it('복사 버튼 클릭 시 클립보드에 복사되어야 함', async () => {
      ...
      jest.mocked(writingExporter.copyToClipboard).mockResolvedValueOnce(true);
