...
type MuiProps = React.PropsWithChildren<Record<string, unknown>>;

// Mock Material-UI
jest.mock('@mui/material', () => ({
  Box: ({ children, ...props }: MuiProps) => <div {...props}>{children}</div>,
  Card: ({ children, ...props }: MuiProps) => <div data-testid="card" {...props}>{children}</div>,
  CardContent: ({ children, ...props }: MuiProps) => <div {...props}>{children}</div>,
  Typography: ({ children, ...props }: MuiProps) => <div {...props}>{children}</div>,
  LinearProgress: (props: Record<string, unknown>) => <div data-testid="linear-progress" {...props} />,
  Chip: ({ label, ...props }: MuiProps & { label?: React.ReactNode }) => <span data-testid="chip" {...props}>{label}</span>,
  Paper: ({ children, ...props }: MuiProps) => <div data-testid="paper" {...props}>{children}</div>,
  Grid: ({ children, ...props }: MuiProps) => <div data-testid="grid" {...props}>{children}</div>,
  Alert: ({ children, ...props }: MuiProps) => <div data-testid="alert" {...props}>{children}</div>,
  AlertTitle: ({ children, ...props }: MuiProps) => <div data-testid="alert-title" {...props}>{children}</div>,
  Collapse: ({ children, in: inProp }: MuiProps & { in?: boolean }) => inProp ? <div>{children}</div> : null,
  IconButton: ({ children, onClick, ...props }: MuiProps & { onClick?: () => void }) => (
...
describe('SystemHealthMonitor', () => {
  ...
  describe('자동 새로고침', () => {
    ...
    it('5초마다 자동으로 헬스 데이터가 갱신되어야 함', async () => {
      ...
      const initialCallCount = jest.mocked(global.fetch).mock.calls.length;

      jest.advanceTimersByTime(5000);

      await waitFor(() => {
        expect(jest.mocked(global.fetch).mock.calls.length).toBeGreaterThan(initialCallCount);
      });
    });
  });
});
