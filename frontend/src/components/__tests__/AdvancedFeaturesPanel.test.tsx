...
describe('AdvancedFeaturesPanel', () => {
  ...
  describe('음성 인식', () => {
    it('음성 인식 시작 버튼이 표시되어야 함', () => {
      ...
      const voiceTab = screen.getByRole('tab', { name: /음성 인식/ });
      fireEvent.click(voiceTab);
  ...
  describe('목소리 생성 다운로드', () => {
    ...
    it('단일 오디오 생성 후 Brainwave 디자인: 성공 메시지·내보내기·편집 버튼 표시', async () => {
      const { speakQwenTtsScriptFromSourceUrl } = require('../../services/qwenTtsService');
      speakQwenTtsScriptFromSourceUrl.mockResolvedValue(new Blob(['audio'], { type: 'audio/mp3' }));
      Object.defineProperty(URL, 'createObjectURL', { value: jest.fn(() => 'blob:mock'), writable: true, configurable: true });
      Object.defineProperty(URL, 'revokeObjectURL', { value: jest.fn(), writable: true, configurable: true });
      ...
      expect(screen.getByText(/음성이 성공적으로 생성되었습니다/)).toBeInTheDocument();
      expect(screen.getByTestId('voice-gen-edit-script')).toBeInTheDocument();
    }, 10000);

    it('편집 버튼 클릭 시 대본 입력란에 포커스', async () => {
      const { speakQwenTtsScriptFromSourceUrl } = require('../../services/qwenTtsService');
      speakQwenTtsScriptFromSourceUrl.mockResolvedValue(new Blob(['audio'], { type: 'audio/mp3' }));
      Object.defineProperty(URL, 'createObjectURL', { value: jest.fn(() => 'blob:mock'), writable: true, configurable: true });
      Object.defineProperty(URL, 'revokeObjectURL', { value: jest.fn(), writable: true, configurable: true });
      ...
      const scriptInput = screen.getByTestId('voice-gen-script');
      const focusSpy = jest.spyOn(scriptInput, 'focus');
      fireEvent.click(screen.getByTestId('voice-gen-edit-script'));
      ...
      await waitFor(() => {
        ...
        const callsWithEmptyId = calls.filter((call: unknown[]) => {
          if (call.length > 0 && typeof call[0] === 'string') {
            return coerceTrimmedString(call[0], '') === '';
          }
          return false;
        });
        // 빈 값으로 호출되지 않았어야 함
