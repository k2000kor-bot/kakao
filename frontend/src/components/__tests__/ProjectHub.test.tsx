...
jest.mock('../ProjectShareDialog', () => {
  return function MockProjectShareDialog({ open, onClose, project }: { open: boolean; onClose: () => void; project?: Project | null }) {
    if (!open) return null;
    return (
...
describe('ProjectHub', () => {
  ...
  describe('기본 렌더링', () => {
    it('기본 렌더링이 올바르게 작동해야 함', () => {
      ...
      expect(screen.getByTestId('project-hub-root')).toBeInTheDocument();
      expect(screen.getByRole('heading', { level: 2, name: '프로젝트 목록' })).toBeInTheDocument();
    });
  ...
  describe('상태 필터링', () => {
    it('활성 프로젝트만 필터링되어야 함', async () => {
      ...
      const statusSelect = statusSelects.find((el: HTMLElement) =>
  ...
  describe('카테고리 필터링', () => {
    it('카테고리로 필터링이 작동해야 함', async () => {
      ...
      const categorySelect = categorySelects.find((el: HTMLElement) => {
