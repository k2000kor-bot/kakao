...
const masterStatusUrl = () => joinApiHealthCheckUrl(API_BASE_URL, API_STATUS_PATH);
const masterAnalyticsUrl = () => joinApiHealthCheckUrl(API_BASE_URL, API_ANALYTICS_PATH);
...
jest.mock('../../utils/rehypeHighlightSearch', () => ({
  rehypeHighlightSearch: () => (_tree: unknown) => {
    /* unist-util-visit는 ESM 전용 — 이 스위트는 마스터 UI만 검증 */
  },
}));
...
describe('IntegratedMasterInterface', () => {
  ...
  describe('기본 렌더링', () => {
    ...
    it('모든 탭이 표시되어야 함', () => {
      ...
      expect(screen.getByText(/통합 대화/i)).toBeInTheDocument();
  ...
  describe('탭 전환', () => {
    it('통합 대화 탭이 기본적으로 활성화되어 있어야 함', () => {
      renderWithTheme(<IntegratedMasterInterface />);

      expect(screen.getByText('통합 AI 대화')).toBeInTheDocument();
  ...
  describe('메시지 입력 및 전송', () => {
    ...
    it('메시지를 전송할 수 있어야 함', async () => {
      ...
      await waitFor(() => {
        ...
        const found = calls.some(
          (c: unknown[]) =>
            typeof c[0] === 'string' && /\/api\/chat$/.test(String(c[0]).split('?')[0])
        );
      ...
      expect(body.quality).toBe('enhanced');
      const ctx = body.context as Record<string, unknown> | undefined;
      expect(ctx?.user_id === 'master_interface' || body.user_id === 'master_interface').toBe(true);
    });
    ...
    it('REACT_APP_GENSPARK_DISABLE_WINDOW_ROUTE_CONTEXT=1이면 URL에 id가 있어도 전송 본문 context에 genspark_*가 없다', async () => {
      ...
      try {
        ...
        renderWithTheme(<IntegratedMasterInterface />);
        const messageInput = screen.getByPlaceholderText(/Type '\/' for commands/);
        fireEvent.change(messageInput, { target: { value: '안녕' } });
    ...
    it('멀티레이어 힌트 env 활성화 시 초장문 전송 시 surface 분석 입력이 상한으로 잘린다', async () => {
      ...
      try {
        renderWithTheme(<IntegratedMasterInterface />);
        const longMsg = 'p'.repeat(CHAT_MULTILAYER_STYLE_HINT_MAX_INPUT_CHARS + 60);
        const messageInput = screen.getByPlaceholderText(/Type '\/' for commands/);
        fireEvent.change(messageInput, { target: { value: longMsg } });
        fireEvent.click(screen.getByRole('button', { name: /메시지 전송/i }));

        await waitFor(() => {
          expect(spy).toHaveBeenCalledWith(
            'p'.repeat(CHAT_MULTILAYER_STYLE_HINT_MAX_INPUT_CHARS),
            'surface'
          );
        });
      } finally {
  ...
  describe('시스템 상태 조회', () => {
    it('시스템 상태를 조회할 수 있어야 함', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(masterStatusUrl());
      });
    });

    it('시스템 상태 조회 실패 시 에러를 로깅해야 함', async () => {
      ...
      const analyticsPayload = {
        ...
        data: {
          total_messages: 100,
          active_users: 10,
          system_performance: { avg_response_time: 200, success_rate: 0.95, uptime: 99.9 },
      ...
      mockFetch.mockImplementation((input: RequestInfo | URL) => {
        ...
        if (url.includes(API_STATUS_PATH)) {
          statusCalls += 1;
          if (statusCalls === 1) {
        ...
        if (url.includes(API_ANALYTICS_PATH)) {
          return Promise.resolve({ ok: true, json: async () => analyticsPayload });
        }
      ...
      await waitFor(() => {
        expect(mockFetch).toHaveBeenCalledWith(masterStatusUrl());
      });
  ...
  describe('분석 데이터 조회', () => {
    it('분석 데이터를 조회할 수 있어야 함', async () => {
      ...
      // 초기 로드 시 분석 데이터 조회
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(masterAnalyticsUrl());
      });
    });

    it('분석 데이터 조회 실패 시 에러를 로깅해야 함', async () => {
      const mockFetch: jest.MockedFunction<typeof fetch> = jest.mocked(global.fetch);
      mockFetch.mockReset();
      mockFetch.mockImplementation((input: RequestInfo | URL) => {
        const url =
