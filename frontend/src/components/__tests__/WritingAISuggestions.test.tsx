 ...
 * AI 글쓰기 제안 기능 확인
 */
/* eslint-disable testing-library/no-node-access, testing-library/no-wait-for-side-effects */
...
describe('WritingAISuggestions', () => {
  ...
  describe('백엔드 API 호출', () => {
    it('백엔드 API를 호출해야 함', async () => {
      ...
      expect(String(url)).toBe(joinApiHealthCheckUrl(resolveApiBaseUrl(), API_V7_WRITING_SUGGESTIONS_PATH));
  ...
  describe('내용 변경', () => {
    it('내용이 변경되면 새로운 제안을 생성해야 함', async () => {
      ...
      const callCountBefore = jest.mocked(global.fetch).mock.calls.length;
      ...
      await waitFor(() => {
        expect(jest.mocked(global.fetch).mock.calls.length).toBeGreaterThan(callCountBefore);
      });
    });
