...
interface ProjectTemplateSelectorProps {
  ...
  onSaveAsTemplate?: (projectData: unknown) => void;
}
...
const ProjectTemplateSelector: React.FC<ProjectTemplateSelectorProps> = ({
  ...
  const loadTemplates = () => {
    ...
    if (q) {
      loadedTemplates = projectTemplateService.searchTemplates(q);
    }
  ...
  const filteredTemplates = templates.filter(template => {
    const trimmedQ = coerceTrimmedString(searchQuery, '');
  ...
  return (
    <>
      <Dialog
        ...
        aria-describedby="project-template-dialog-content"
        role="dialog"
      >
        <DialogTitle id="project-template-dialog-title">
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Typography variant="h6">프로젝트 템플릿 선택</Typography>
            <IconButton size="small" onClick={onClose} aria-label="템플릿 선택 닫기" type="button">
              <Box component="span" sx={{ fontSize: 20 }} aria-hidden="true">×</Box>
            </IconButton>
          </Box>
        </DialogTitle>

        <DialogContent id="project-template-dialog-content" dividers>
          {/* 검색 */}
          <TextField
            fullWidth
            placeholder="템플릿 검색..."
                    ...
                    <CardContent>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                        ...
                        <IconButton
                          ...
                          aria-label={`${template.name} 템플릿 메뉴`}
                        >
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
                        ...
                        <Button
                          ...
                          aria-label={`${template.name} 템플릿 사용하기`}
                        >
                          사용하기
                        </Button>
                      </Box>
        ...
        <DialogActions>
          <Button type="button" onClick={onClose} aria-label="취소">취소</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog
        ...
        aria-describedby="delete-template-dialog-content"
      >
        <DialogTitle id="delete-template-dialog-title">템플릿 삭제</DialogTitle>
        <DialogContent id="delete-template-dialog-content">
          <Typography>
            정말로 "{selectedTemplate?.name}" 템플릿을 삭제하시겠습니까?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button type="button" onClick={() => setShowDeleteConfirm(false)} aria-label="취소">취소</Button>
          <Button type="button" onClick={handleDeleteTemplate} color="error" variant="contained" aria-label="템플릿 삭제">
            삭제
          </Button>
        </DialogActions>
      </Dialog>
