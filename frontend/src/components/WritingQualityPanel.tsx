...
/** writingQualityAnalyzer.analyzeStyle 반환 타입 */
...
const WritingQualityPanel: React.FC<WritingQualityPanelProps> = ({ content, onImprove }) => {
  ...
  useEffect(() => {
    ...
    if (trimmed.length > 0) {
      const qualityAnalysis = writingQualityAnalyzer.analyzeQuality(trimmed);
      const style = writingQualityAnalyzer.analyzeStyle(trimmed);
  ...
  if (!analysis || !coerceTrimmedString(content, '')) {
    return (
  ...
  const getScoreColor = (score: number): string => getQualityScoreColor(score);
