...
interface MarketingAnalysisResult {
    total_marketing_score: number;
    hashtag_count: number;
    emoji_count: number;
    emotion_tone: string;
    marketing_elements: Record<string, number>;
    readability: { word_count: number; sentence_count: number; avg_words_per_sentence: number };
    suggestions?: string[];
}
...
const MarketingContent: React.FC<MarketingContentProps> = ({ onContentGenerated }) => {
    ...
    const generateSocialContent = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_MARKETING_SOCIAL_PATH), {
                method: 'POST',
                headers: {
    ...
    const generateEmailContent = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_MARKETING_EMAIL_PATH), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const analyzeMarketingContent = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_MARKETING_ANALYZE_PATH), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            const data = (await response.json()) as { success: boolean; data?: MarketingAnalysisResult; error?: string };
    ...
    const renderEmailTab = () => (
        <Box>
            ...
            <Button
                variant="contained"
                color="primary"
                onClick={() => void generateEmailContent()}
                disabled={loading}
                startIcon={<Email />}
                sx={{ mb: 2 }}
            >
        ...
        <Box>
            <Stack spacing={2}>
                ...
                <Button
                    variant="contained"
                    color="warning"
                    onClick={() => void analyzeMarketingContent()}
