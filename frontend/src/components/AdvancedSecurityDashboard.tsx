...
const AdvancedSecurityDashboard: React.FC = () => {
    ...
    const loadSecurityData = async () => {
        try {
            const [threatsRes, eventsRes, keysRes, logsRes, statusRes] = await Promise.all([
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_THREATS_PATH)),
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_EVENTS_PATH)),
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_KEYS_PATH)),
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_AUDIT_LOGS_PATH)),
                axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_STATUS_PATH))
            ]);
    ...
    const handleEncryptData = async () => {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_ENCRYPT_PATH), {
                data: encryptData
            });
    ...
    const handleDecryptData = async () => {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_DECRYPT_PATH), {
    ...
    const handleSecurityScan = async () => {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, ADVANCED_SECURITY_SCAN_PATH), {
                [API_QUERY_PARAM_SCAN_TYPE]: scanType,
            });
            if (response.data.success && response.data.data) {
                setScanResult(response.data.data as ScanResult);
                setError(null);
                setScanDialog(false);
                loadSecurityData();
            }
    ...
    const handleResolveThreat = async (threatId: string) => {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, `${ADVANCED_SECURITY_THREATS_PATH}/${encodeURIComponent(threatId)}/resolve`), {
    ...
    const getRiskLevelColor = (riskLevel: string): 'error' | 'warning' | 'success' | 'default' => {
        switch (riskLevel) {
