...
import { USER_PREFERENCES_JSON_STORAGE_KEY } from '../services/userPreferencesUiStorageKeys';
...
const UserSettings: React.FC<UserSettingsProps> = ({ onClose }) => {
  ...
  const { t: _t, changeLanguage } = useTranslation();
  ...
  useEffect(() => {
    // 로컬 스토리지에서 설정 로드
    const saved = localStorage.getItem(USER_PREFERENCES_JSON_STORAGE_KEY);
    if (saved) {
      try {
  ...
  const handlePreferenceChange = useCallback((section: keyof UserPreferences, key: string, value: unknown) => {
    setPreferences((prev) => ({
      ...prev,
      [section]: {
  ...
  const handleReset = useCallback(() => {
    if (globalThis.window?.confirm?.('모든 설정을 초기화하시겠습니까?')) {
      localStorage.removeItem(USER_PREFERENCES_JSON_STORAGE_KEY);
