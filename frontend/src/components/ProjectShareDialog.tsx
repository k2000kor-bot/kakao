...
const ProjectShareDialog: React.FC<ProjectShareDialogProps> = ({
    ...
    const [_selectedShare, _setSelectedShare] = useState<ProjectShareLink | null>(null);
    ...
    const handleCreateShare = () => {
        const _shareLink = projectShareService.createShareLink(projectId, {
    ...
    const handleDeleteShare = (shareId: string) => {
        setDeleteConfirmShareId(shareId);
    };

    const confirmDeleteShare = () => {
        if (deleteConfirmShareId) {
            projectShareService.deleteShareLink(deleteConfirmShareId);
            loadShareLinks();
    ...
    return (
        ...
        >
            <DialogTitle>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <IconButton
                        ...
                        size="small"
                        onClick={onClose}
                        aria-label="프로젝트 공유 모달 닫기"
                        className="project-share-dialog-close"
            ...
            <DialogContent dividers>
                ...
                {!showCreateForm && (
                    <Box sx={{ mb: 3 }}>
                        <Button
                            ...
                            fullWidth
                            data-testid="project-share-create-btn"
                        >
                            새 공유 링크 생성
                        </Button>
                    </Box>
                ...
                {/* 공유 링크 생성 폼 */}
                {showCreateForm && (
                    <Box sx={{ mb: 3, p: 2, bgcolor: 'var(--bg-secondary)', borderRadius: 1 }}>
                        ...
                        <FormControl fullWidth sx={{ mb: 2 }}>
                            ...
                            <Select
                                ...
                                label="권한"
                                onChange={(e) => setNewShare({ ...newShare, permission: e.target.value as 'read' | 'write' | 'admin' })}
                            >
                        ...
                        <Box sx={{ display: 'flex', gap: 1 }}>
                            ...
                            <Button type="button" onClick={() => setShowCreateForm(false)} aria-label="공유 링크 생성 취소">취소</Button>
                        </Box>
                    </Box>
                )}
            ...
            <DialogActions>
                <Button type="button" onClick={onClose} aria-label="공유 모달 닫기">닫기</Button>
            </DialogActions>
        </Dialog>

        {/* 공유 링크 삭제 확인 모달 */}
        <Dialog
            open={!!deleteConfirmShareId}
            onClose={() => setDeleteConfirmShareId(null)}
            onKeyDown={(e) => { if (e.key === 'Escape') setDeleteConfirmShareId(null); }}
        >
            <DialogTitle>공유 링크 삭제</DialogTitle>
            <DialogContent>
                <Typography>정말로 이 공유 링크를 삭제하시겠습니까?</Typography>
            </DialogContent>
            <DialogActions>
                <Button type="button" onClick={() => setDeleteConfirmShareId(null)} aria-label="삭제 취소">취소</Button>
                <Button type="button" onClick={confirmDeleteShare} color="error" variant="contained" aria-label="공유 링크 삭제 확인">삭제</Button>
            </DialogActions>
        </Dialog>
        ...
        <Snackbar
            ...
            anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}
        >
            <Alert onClose={() => setDeleteSnackbarOpen(false)} severity="success" variant="filled">
                공유 링크가 삭제되었습니다
            </Alert>
        </Snackbar>
        </>
