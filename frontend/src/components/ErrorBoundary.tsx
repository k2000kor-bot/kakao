...
class ErrorBoundary extends Component<Props, State> {
  ...
  logErrorToService = (error: Error, errorInfo: ErrorInfo) => {
    ...
    try {
      ...
      const errors = JSON.parse(localStorage.getItem(UI_ERROR_LOG_STORAGE_KEY) || '[]');
      errors.push(errorData);
      if (errors.length > 50) {
        errors.shift(); // 최대 50개만 유지
      }
      localStorage.setItem(UI_ERROR_LOG_STORAGE_KEY, JSON.stringify(errors));
    } catch (e) {
    ...
    if (this.state.hasError) {
      ...
      return (
        <div 
          ...
          aria-live="assertive"
          aria-atomic="true"
          data-testid={TEST_IDS.ERROR_BOUNDARY}
        >
