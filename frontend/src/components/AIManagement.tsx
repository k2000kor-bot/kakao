...
import {
    ...
    INTEGRATED_API_AI_FEEDBACK_PATH,
...
function AIManagement({ onOptimizationComplete }: AIManagementProps) {
    ...
    const runOptimization = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_AI_OPTIMIZE_PATH), {
                method: 'POST',
                headers: {
    ...
    const runBenchmark = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_AI_BENCHMARK_PATH), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const submitFeedback = async () => {
        const content = coerceTrimmedString(feedbackContent, '');
        if (!content) return;
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_AI_FEEDBACK_PATH), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                ...
                <Paper sx={{ p: 2, mt: 2 }}>
                    ...
                    <Grid container spacing={2} sx={{ mb: 2 }}>
                        <Grid size={{ xs: 6 }}>
                            ...
                            <Box sx={{ bgcolor: 'var(--bg-tertiary)', p: 1, borderRadius: 1 }}>
    ...
    const renderFeedbackTab = () => (
        <Box>
            ...
            <Button
                ...
                disabled={loading || !coerceTrimmedString(feedbackContent, '')}
                startIcon={<Feedback />}
                sx={{ mb: 2 }}
            >
                {loading ? '처리 중...' : '피드백 제출'}
