...
import { CREATIVE_WRITINGS_STORAGE_KEY } from '../services/writingUiStorageKeys';
...
const CreativeWriting: React.FC<CreativeWritingProps> = ({ onContentGenerated }) => {
    ...
    const generateStory = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_CREATIVE_STORY_PATH), {
                method: 'POST',
                headers: {
    ...
    const generatePoem = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_CREATIVE_POEM_PATH), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const generateEssay = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_CREATIVE_ESSAY_PATH), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const analyzeWriting = async () => {
        ...
        try {
            const response = await fetch(joinApiHealthCheckUrl(resolveApiBaseUrl(), INTEGRATED_API_CREATIVE_ANALYZE_PATH), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            const data = (await response.json()) as { success: boolean; data?: AnalysisResult; error?: string };
    ...
    const handleSave = () => {
        if (generatedContent) {
            try {
                const savedWritings = JSON.parse(localStorage.getItem(CREATIVE_WRITINGS_STORAGE_KEY) || '[]');
                const newWriting = {
                    id: Date.now(),
                ...
                localStorage.setItem(CREATIVE_WRITINGS_STORAGE_KEY, JSON.stringify(savedWritings));
                setError('');
                // 성공 알림은 Alert로 표시
            } catch (err) {
