...
describe('formatters', () => {
  describe('formatDate', () => {
    ...
    it('relative 형식에서 60초 미만이면 "방금 전"을 반환해야 함', () => {
      const justNow = new Date(Date.now() - 30 * 1000);
      const result = formatDate(justNow, 'relative');
      expect(result).toBe('방금 전');
    });

    it('문자열 날짜를 short 형식으로 포맷해야 함', () => {
      const result = formatDate('2024-06-01T12:00:00', 'short');
      expect(result).toContain('2024');
      expect(result).toMatch(/\d/);
    });

    it('relative 형식에서 7일 이상이면 short 형식으로 반환해야 함', () => {
      const longAgo = new Date(Date.now() - 8 * 24 * 60 * 60 * 1000);
      const result = formatDate(longAgo, 'relative');
  ...
  describe('truncateText', () => {
    ...
    it('커스텀 suffix를 사용할 수 있어야 함', () => {
      const text = 'Long text here';
      const result = truncateText(text, 5, '--');
      // maxLength 5 - suffix 2 = 3자 + '--' => 'Lon--'
