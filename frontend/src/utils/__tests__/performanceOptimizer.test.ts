...
// IntersectionObserver 모킹 — `new` + toBeInstanceOf(IntersectionObserver) 호환
...
describe('performanceOptimizer', () => {
  ...
  describe('optimizeTextRendering', () => {
    /* optimizeTextRendering 반환값은 render() 결과가 아님 */
    /* eslint-disable testing-library/render-result-naming-convention */
    it('짧은 텍스트는 그대로 반환해야 함', () => {
      const text = 'Short text';
      const shortened = optimizeTextRendering(text, 100);

      expect(shortened).toBe(text);
    });

    it('긴 텍스트는 요약해야 함', () => {
      const longText = 'a'.repeat(2000);
      const shortened = optimizeTextRendering(longText, 1000);

      expect(shortened.length).toBe(1003); // 1000 + '...'
      expect(shortened).toContain('...');
    });

    it('기본 maxLength를 사용해야 함', () => {
      const longText = 'a'.repeat(2000);
      const shortened = optimizeTextRendering(longText);

      expect(shortened.length).toBe(1003); // 기본값 1000 + '...'
    });
