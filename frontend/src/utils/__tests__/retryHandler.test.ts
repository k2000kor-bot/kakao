...
import { retry, retryApiCall, NetworkMonitor, retryWithNetworkCheck } from '../retryHandler';
...
describe('retryHandler', () => {
  ...
  describe('retry', () => {
    ...
    it('재시도 불가능한 에러는 즉시 실패해야 함', async () => {
      ...
      const result = await retry(mockFn, {
        maxRetries: 3,
        initialDelay: 10,
        retryable: (error) => !(error as Error).message.includes('Auth'),
      });
    ...
    it('retryable 미지정 시 429 에러는 재시도해야 함', async () => {
      let callCount = 0;
      const mockFn = jest.fn().mockImplementation(() => {
        callCount++;
        if (callCount < 2) {
          return Promise.reject(Object.assign(new Error('Too Many Requests'), { response: { status: 429 } }));
        }
    ...
    it('retryable 미지정 시 5xx 에러는 재시도해야 함', async () => {
      let callCount = 0;
      const mockFn = jest.fn().mockImplementation(() => {
        callCount++;
        if (callCount < 2) {
          return Promise.reject(Object.assign(new Error('Server Error'), { response: { status: 503 } }));
        }
        return Promise.resolve('ok');
      });
    ...
    it('retryable 미지정 시 Failed to fetch 에러는 재시도해야 함', async () => {
      let callCount = 0;
      const mockFn = jest.fn().mockImplementation(() => {
        callCount++;
        if (callCount < 2) {
      ...
      const result = await retry(mockFn, {
        maxRetries: 2,
        initialDelay: 10,
        // retryable 생략 → defaultRetryable 사용 (Failed to fetch → true)
      });
  ...
  describe('retryWithNetworkCheck', () => {
    it('온라인일 때 retry와 동일하게 성공 결과를 반환해야 함', async () => {
      const mockFn = jest.fn().mockResolvedValue('data');
      const result = await retryWithNetworkCheck(mockFn, {
        maxRetries: 1,
        initialDelay: 10,
      });
