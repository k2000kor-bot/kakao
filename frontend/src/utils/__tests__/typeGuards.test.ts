...
import type { Message, AnalysisData, ChatAPIResponse, EmotionAnalysis, IntentAnalysis } from '../../types';
...
describe('typeGuards', () => {
  describe('isMessage', () => {
    ...
    it('analysis가 유효하지 않은 객체면 거부해야 함', () => {
      const messageWithInvalidAnalysis = {
        id: 1,
        sender: 'user' as const,
        text: 'Hello',
        timestamp: '2024-01-01T00:00:00Z',
        analysis: {}, // isAnalysisData({}) === false
      };

      expect(isMessage(messageWithInvalidAnalysis)).toBe(false);
    });
  });
