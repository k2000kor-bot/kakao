...
global.PerformanceObserver = jest.fn().mockImplementation((_callback: PerformanceObserverCallback) => {
  return {
    observe: jest.fn(),
    disconnect: jest.fn(),
    takeRecords: jest.fn(() => []),
...
describe('PerformanceMonitor', () => {
  ...
  describe('getNetworkPerformance', () => {
    ...
    it('네트워크 정보가 없으면 빈 객체를 반환해야 함', () => {
      ...
      const originalConnection = nav.connection;
      delete nav.connection;

      const network = performanceMonitor.getNetworkPerformance();
      expect(network.connection).toBeUndefined();

      nav.connection = originalConnection;
    });
  });
  ...
  describe('generateReport', () => {
    it('초기 상태(메트릭 없음)에서 summary 구조를 반환해야 함', () => {
      performanceMonitor.clear();
      const report = performanceMonitor.generateReport();

      expect(report.summary).toBeDefined();
      expect(report.summary.totalMetrics).toBe(0);
      expect(report.summary.slowComponents).toEqual([]);
