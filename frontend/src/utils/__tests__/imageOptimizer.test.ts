...
describe('imageOptimizer', () => {
  ...
  describe('getOptimizedImageUrl', () => {
    ...
    it('프로토콜 상대 URL(//)에도 파라미터를 추가해야 함', () => {
      const url = getOptimizedImageUrl('//cdn.example.com/image.jpg', {
        maxWidth: 640,
        quality: 0.7,
      });
      ...
      expect(url).toMatch(/^\/\/cdn\.example\.com\/image\.jpg[?&]/);
    });
    ...
    it('WebP 미지원이면 format이 webp여도 f=webp를 추가하지 않아야 함', () => {
      const url = getOptimizedImageUrl('https://example.com/image.jpg', {
        format: 'webp',
        webpSupported: false,
      });
  ...
  describe('isWebPSupported', () => {
    it('WebP 지원 여부를 확인해야 함', () => {
      const createElementOriginal = document.createElement.bind(document);
      document.createElement = jest.fn((tagName: string) => {
        const el = createElementOriginal(tagName);
        if (tagName.toLowerCase() === 'canvas') {
          (el as HTMLCanvasElement).toDataURL = jest.fn((type?: string) =>
            type === 'image/webp' ? 'data:image/webp;base64,test' : 'data:image/png;base64,'
          );
        }
        return el;
      });
  ...
  describe('getImageDimensions', () => {
    // FileReader/Image 비동기 체인이 Jest/jsdom에서 완료되지 않아 스킵 (E2E에서 검증)
    ...
    it('이미지 로드 실패 시 에러를 던져야 함', async () => {
      ...
      (global as unknown as { Image: typeof Image }).Image = jest.fn().mockImplementation(() => {
        const img = { width: 0, height: 0, src: '', onload: null, onerror: null };
        Object.defineProperty(img, 'src', {
          set: function () {
            setTimeout(() => {
      ...
      (global as unknown as { Image: typeof Image }).Image = origImage;
      jest.useRealTimers();
    });
  });

  // FileReader/Image/Canvas 비동기 체인이 Jest/jsdom에서 완료되지 않아 스킵 (E2E에서 검증)
