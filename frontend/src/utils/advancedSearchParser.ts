...
class AdvancedSearchParser {
  ...
  parseQuery(query: string, options: SearchOptions = {}): SearchQuery {
    const trimmed = coerceTrimmedString(query, '');
  ...
  private tokenize(query: string): string[] {
    ...
    for (let i = 0; i < query.length; i++) {
      ...
      if ((char === '"' || char === "'") && (i === 0 || query[i - 1] === ' ' || query[i - 1] === '(')) {
        if (!inQuotes) {
          ...
          inQuotes = false;
          quoteChar = '';
          const quotedTok = coerceTrimmedString(current, '');
          if (quotedTok) {
            tokens.push(quotedTok);
            current = '';
          }
          continue;
        }
      ...
      if (char === ' ' && (nextChar === ' ' || i === query.length - 1)) {
        const spaceTok = coerceTrimmedString(current, '');
        if (spaceTok) {
          tokens.push(spaceTok);
          current = '';
        }
        continue;
      }
      ...
      if (andMatch) {
        const andTok = coerceTrimmedString(current, '');
        if (andTok) {
          tokens.push(andTok);
          current = '';
        }
        ...
        const orTok = coerceTrimmedString(current, '');
        if (orTok) {
          tokens.push(orTok);
          current = '';
        }
        ...
        const notTok = coerceTrimmedString(current, '');
        if (notTok) {
          tokens.push(notTok);
          current = '';
        }
    ...
    const finalTok = coerceTrimmedString(current, '');
    if (finalTok) {
      tokens.push(finalTok);
    }
