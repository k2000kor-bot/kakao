...
class NotificationService {
    ...
    private initializeAudio(): void {
        try {
            ...
            errorLogger.warn('오디오 컨텍스트 초기화 실패', { component: 'NotificationService', action: 'initializeAudio', error: String(error) });
        }
    }
    ...
    private async requestNotificationPermission(): Promise<void> {
        if ('Notification' in window && Notification.permission === 'default') {
            try {
                ...
                errorLogger.warn('알림 권한 요청 실패', { component: 'NotificationService', action: 'requestNotificationPermission', error: String(error) });
            }
        }
    }
    ...
    private loadSettings(): void {
        try {
            ...
            errorLogger.warn('알림 설정 로드 실패', { component: 'NotificationService', action: 'loadSettings', error: String(error) });
        }
    }
    ...
    private saveSettings(): void {
        try {
            ...
            errorLogger.warn('알림 설정 저장 실패', { component: 'NotificationService', action: 'saveSettings', error: String(error) });
        }
    }
    ...
    private playNotificationSound(type: string): void {
        ...
        try {
            ...
            errorLogger.warn('알림 사운드 재생 실패', { component: 'NotificationService', action: 'playNotificationSound', error: String(error) });
        }
    }
    ...
    private showDesktopNotification(notification: NotificationData): void {
        ...
        try {
            ...
            errorLogger.warn('데스크톱 알림 표시 실패', { component: 'NotificationService', action: 'showDesktopNotification', error: String(error) });
        }
    }
...
// 싱글톤 인스턴스
export { NOTIFICATION_SETTINGS_STORAGE_KEY } from './notificationStorageKeys';
