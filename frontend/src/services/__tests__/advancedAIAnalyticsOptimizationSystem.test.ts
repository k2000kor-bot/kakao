...
import advancedAIAnalyticsOptimizationSystem, { AIAnalyticsData } from '../advancedAIAnalyticsOptimizationSystem';
...
describe('advancedAIAnalyticsOptimizationSystem', () => {
  ...
  describe('approveOptimization', () => {
    it('최적화를 승인할 수 있어야 함', async () => {
      ...
      expect(Array.isArray(recommendations)).toBe(true);
      /* eslint-disable jest/no-conditional-expect -- API may return empty; approval tested when present */
      if (recommendations.length > 0) {
        const listener = jest.fn();
