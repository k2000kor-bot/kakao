...
const MOCK_MSG_TS = '2025-01-01T00:00:00.000Z';
...
describe('multiStepResponseGenerator', () => {
    ...
    describe('executeResponseStep', () => {
        ...
        it('단계를 실행해야 함', async () => {
            ...
            expect((result as StepResultShape).stepId).toBe(mockStep.id);
            expect((result as StepResultShape).result).toBe('테스트 응답');
            expect(sendChatMessage).toHaveBeenCalled();
            const firstCall = jest.mocked(sendChatMessage).mock.calls[0][0];
            expect(firstCall.conversation_history).toBeUndefined();
        });

        it('완료된 이전 단계가 있으면 conversation_history를 넣는다', async () => {
            jest.mocked(sendChatMessage).mockResolvedValueOnce({
                success: true,
                message: { content: '2단계 응답', timestamp: MOCK_MSG_TS },
            });

            const prevResults = {
                'analysis-step': {
                    stepId: 'analysis-step',
                    stepName: ASSISTANT_GENERATION_STEP_LABELS_DEFAULT[0],
                    result: '이전 분석 요약',
                },
            };

            await executeResponseStep(
                mockStep,
                '질문: A\n요구사항: B',
                {},
                prevResults,
            ...
            const hist = arg.conversation_history;
            expect(Array.isArray(hist)).toBe(true);
            expect(hist!.length).toBeGreaterThanOrEqual(2);
            expect(
                String(hist![1]?.content ?? '').includes('이전 분석')
            ).toBe(true);
        });
        ...
        it('분석 단계를 실행해야 함', async () => {
            ...
            expect(result).toBeDefined();
            expect((result as StepResultShape).result).toBe('분석 결과');
        });
        ...
        it('조사 단계를 실행해야 함', async () => {
            ...
            expect(result).toBeDefined();
            expect((result as StepResultShape).result).toBe('조사 결과');
        });
        ...
        it('검증 단계를 실행해야 함', async () => {
            ...
            expect(result).toBeDefined();
            expect((result as StepResultShape).result).toBe('검증 결과');
        });
        ...
        it('포맷팅 단계를 실행해야 함', async () => {
            ...
            expect(result).toBeDefined();
            expect((result as StepResultShape).result).toBe('포맷팅된 결과');
        });
        ...
        it('API 실패 시 에러를 처리해야 함', async () => {
            ...
            expect(result).toBeDefined();
            expect((result as StepResultShape).error).toBeDefined();
        });
