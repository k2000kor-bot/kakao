...
// File 객체 생성 헬퍼
function createMockFile(name: string, type: string, size: number, _content?: string): File {
...
describe('clientFileProcessor', () => {
  ...
  describe('processFile', () => {
    ...
    it('CSV 파일을 텍스트로 읽어 처리할 수 있어야 함', async () => {
      const file = createMockFile('data.csv', 'text/csv', 80);
      const projectId = 'test-project';
  ...
  describe('initializeDaewooConstructionKnowledge', () => {
    it('건설·시공 샘플 지식을 초기화할 수 있어야 함', async () => {
      ...
      if (kb) {
        ...
        expect(kb.categories['시공사']).toBeDefined();
      }
    });
  });
