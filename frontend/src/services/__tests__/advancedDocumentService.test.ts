...
describe('advancedDocumentService', () => {
  ...
  describe('analyzeLongConversation', () => {
    it('긴 대화를 분석할 수 있어야 함', async () => {
      ...
      const messages: Message[] = [
        { id: '1', role: 'user', sender: 'user', content: '메시지 1', timestamp: new Date().toISOString() },
        { id: '2', role: 'assistant', sender: 'assistant', content: '응답 1', timestamp: new Date().toISOString() }
      ];
  ...
  describe('generateContextualResponse', () => {
    it('맥락 유지 응답을 생성할 수 있어야 함', async () => {
      ...
      const messages: Message[] = [
        { id: '1', role: 'user', sender: 'user', content: '이전 메시지', timestamp: new Date().toISOString() }
      ];
  ...
  describe('analyzeStyleConsistency', () => {
    it('스타일 일관성을 분석할 수 있어야 함', async () => {
      ...
      const messages: Message[] = [
        { id: '1', role: 'user', sender: 'user', content: '메시지 1', timestamp: new Date().toISOString(), isUser: true },
        { id: '2', role: 'assistant', sender: 'assistant', content: '응답 1', timestamp: new Date().toISOString(), isUser: false },
        { id: '3', role: 'user', sender: 'user', content: '메시지 2', timestamp: new Date().toISOString(), isUser: true },
        { id: '4', role: 'user', sender: 'user', content: '메시지 3', timestamp: new Date().toISOString(), isUser: true }
      ];
  ...
  describe('실제 사용자 시나리오 테스트', () => {
    it('재개발 프로젝트 관련 문서를 처리할 수 있어야 함', async () => {
      ...
      const request: AdvancedDocumentRequest = {
        documentText: '샘플 재개발 프로젝트 시공사 선정 관련 문서',
        conversationHistory: [],
