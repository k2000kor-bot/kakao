...
const mockSendChat: jest.MockedFunction<typeof sendChatMessage> = jest.mocked(sendChatMessage);
...
질문 의도 분석: 도시정비법에 따른 재개발 절차에 대한 상세 정보 요청
            ...
            mockSendChat.mockResolvedValueOnce(mockResponse);
        ...
        it('API 호출 실패 시 기본값을 반환해야 함', async () => {
            mockSendChat.mockRejectedValueOnce(new Error('API 오류'));
    ...
    describe('performWebSearch', () => {
        it('웹 검색을 성공적으로 수행해야 함', async () => {
            ...
            mockSendChat.mockResolvedValue(mockResponse);
        ...
        it('검색 실패 시 빈 배열을 반환해야 함', async () => {
            mockSendChat.mockRejectedValue(new Error('검색 오류'));
    ...
    describe('performFactChecking', () => {
        ...
        it('API 호출 실패 시 기본 메시지를 반환해야 함', async () => {
            mockSendChat.mockRejectedValueOnce(new Error('오류'));
        ...
        it('conversation_history에 pipelineExtras를 실어 sendChatMessage에 전달한다 (merge는 unifiedAPI 본문 빌더에서)', async () => {
                ...
                const hist: ChatTurn[] = [
                    {
                        role: 'assistant',
                        content: '이전 답',
                        pipelineExtras: { generationScenarioMarkdown: '## ikp-scenario\n본문' },
                    },
                ];
                await performFactChecking('테스트 응답', mockSources, hist);
                const req = mockSendChat.mock.calls[0][0];
                const conv = req.conversation_history;
                expect(Array.isArray(conv)).toBe(true);
                expect(conv?.length).toBeGreaterThan(0);
                const hasScenario = (conv ?? []).some((t: ChatTurn) =>
                    String(t.pipelineExtras && t.pipelineExtras.generationScenarioMarkdown).includes(
                        'ikp-scenario'
                    )
                );
    ...
    describe('processIntelligentKnowledge', () => {
        beforeEach(() => {
            // 기본 모킹 설정
            mockSendChat.mockImplementation((request) => {
