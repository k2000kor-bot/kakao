...
import { PWAService, pwaService, PWAConfig, NotificationOptions } from '../pwaService';
...
const mockServiceWorkerRegistration: Partial<ServiceWorkerRegistration> & {
  active: { postMessage: jest.Mock; state: string };
  pushManager: { subscribe: jest.Mock };
...
// caches 모킹 (CacheStorage: keys, delete, match, has, open only)
global.caches = {
...
describe('PWAService', () => {
  ...
  beforeEach(() => {
    ...
    // navigator 재설정
    (global.navigator as unknown as { onLine: boolean }).onLine = true;
    (global.navigator as unknown as Record<string, unknown>).serviceWorker = mockServiceWorker;
  ...
  describe('설치 프롬프트', () => {
    it('설치 프롬프트 표시', async () => {
      ...
      } as unknown as Event & { prompt: jest.Mock; userChoice: Promise<{ outcome: string }> };
  ...
  describe('푸시 알림', () => {
    it('알림 권한 요청', async () => {
      ...
      (global.Notification as unknown as Record<string, unknown>).requestPermission = requestPermissionMock;
    ...
    it('푸시 구독', async () => {
      ...
      (service as unknown as Record<string, unknown>).registration = mockServiceWorkerRegistration;
      (service as unknown as Record<string, unknown>).config.vapidPublicKey = 'dGVzdC1rZXk='; // base64 인코딩된 키
      mockServiceWorkerRegistration.pushManager.subscribe.mockResolvedValue(mockSubscription);
      jest.mocked(global.window.atob).mockReturnValue('test-key');
    ...
    it('VAPID 키 없을 때 null 반환', async () => {
      (service as unknown as Record<string, unknown>).registration = mockServiceWorkerRegistration;
      (service as unknown as Record<string, unknown>).config.vapidPublicKey = undefined;
  ...
  describe('알림 표시', () => {
    it('알림 표시 성공', async () => {
      ...
      (global.Notification as unknown as Record<string, unknown>).requestPermission = jest.fn().mockResolvedValue('granted');
    ...
    it('권한 없을 때 알림 표시 안 함', async () => {
      (service as unknown as Record<string, unknown>).registration = mockServiceWorkerRegistration;
      (global.Notification as unknown as Record<string, unknown>).requestPermission = jest.fn().mockResolvedValue('denied');
    ...
    it('Service Worker 없을 때 알림 표시 안 함', async () => {
      mockShowNotification.mockClear();
      (service as unknown as Record<string, unknown>).registration = null;
  ...
  describe('백그라운드 동기화', () => {
    it('백그라운드 동기화 등록', async () => {
      (service as unknown as Record<string, unknown>).registration = {
      ...
      expect((service as unknown as Record<string, unknown>).registration.sync.register).toHaveBeenCalledWith('sync-tag');
    });

    it('sync API 미지원 시 경고', async () => {
      const regWithoutSync = { ...mockServiceWorkerRegistration };
      delete (regWithoutSync as Record<string, unknown>).sync;
      (service as unknown as Record<string, unknown>).registration = regWithoutSync;
  ...
  describe('오프라인 큐', () => {
    it('오프라인 큐에 요청 추가', () => {
      service.addToOfflineQueue(API_SMOKE_TEST_PATH, { method: 'POST' });

      expect(errorLogger.info).toHaveBeenCalled();
    });

    it('온라인 시 오프라인 큐 처리', async () => {
      jest.mocked(global.fetch).mockResolvedValue({ ok: true });

      service.addToOfflineQueue(`${API_SMOKE_TEST_PATH}1`, { method: 'GET' });
      service.addToOfflineQueue(`${API_SMOKE_TEST_PATH}2`, { method: 'POST' });
  ...
  describe('캐시 관리', () => {
    it('캐시 정리', async () => {
      jest.mocked(global.caches.keys).mockResolvedValue(['cache-1', 'cache-2']);
      jest.mocked(global.caches.delete).mockResolvedValue(true);
  ...
  describe('에지 케이스', () => {
    it('Service Worker 미지원 환경', () => {
      const originalServiceWorker = (global.navigator as unknown as Record<string, unknown>).serviceWorker;
      delete (global.navigator as unknown as Record<string, unknown>).serviceWorker;
      ...
      // 복원
      (global.navigator as unknown as Record<string, unknown>).serviceWorker = originalServiceWorker;
    });
