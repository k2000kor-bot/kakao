...
describe('ConstructionAnalyticsService', () => {
  ...
  describe('초기화', () => {
    ...
    it('초기 상태는 companies_count 0, has_companies_data false', () => {
      const state = service.getCurrentState();

      expect(state.companies_count).toBe(0);
      expect(state.has_companies_data).toBe(false);
    });
  });
  ...
  describe('로컬 데이터 접근', () => {
    ...
    it('getCompanyDetailAnalysis - 데이터 없을 때 null 반환', () => {
      const detail = service.getCompanyDetailAnalysis('nonexistent');
