...
describe('enhancedFileAnalysisService', () => {
  ...
  describe('analyzeFile', () => {
    ...
    it('파일을 분석할 수 있어야 함', async () => {
      ...
      jest.mocked(unifiedAPI.uploadFile).mockResolvedValueOnce({
        success: true,
        file_id: 'file-123',
      });

      jest.mocked(unifiedAPI.processFile).mockResolvedValueOnce({
        success: true,
        data: {
    ...
    it('예외 발생 시 에러를 처리해야 함', async () => {
      ...
      jest.mocked(unifiedAPI.uploadFile).mockRejectedValueOnce(new Error('네트워크 오류'));
  ...
  describe('analyzeImage', () => {
    ...
    beforeEach(() => {
      ...
      interface MockFR { result: string | null; onload: (() => void) | null; readAsDataURL: jest.Mock }
      mockFileReader.readAsDataURL = jest.fn(function(this: MockFR) {
        setTimeout(() => {
    ...
    it('이미지를 분석할 수 있어야 함', async () => {
      ...
      jest.mocked(unifiedAPI.analyzeImage).mockResolvedValueOnce({
        success: true,
        data: {
          analysis: {
  ...
  describe('실제 사용자 시나리오 테스트', () => {
    it('재개발 프로젝트 문서를 분석할 수 있어야 함', async () => {
      const file = new File(
        ['샘플 재개발 프로젝트 제안서입니다. 시공사 선정 기준은 기술력, 안전성, 경험입니다.'],
        '제안서.pdf',
