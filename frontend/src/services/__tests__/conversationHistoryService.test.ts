...
describe('conversationHistoryService', () => {
  ...
  describe('getContextForLLM', () => {
    it('메시지 순서가 유지되어야 함', () => {
      const conversation = conversationHistoryService.createConversation('대화');
      conversationHistoryService.addMessage(conversation.id, { role: 'user', content: '1' });
      conversationHistoryService.addMessage(conversation.id, { role: 'assistant', content: '2' });
      conversationHistoryService.addMessage(conversation.id, { role: 'user', content: '3' });

      const context = conversationHistoryService.getContextForLLM(conversation.id);
      expect(context.map(c => c.content)).toEqual(['1', '2', '3']);
    });
    ...
    it('메시지가 없는 대화는 빈 배열을 반환해야 함', () => {
      const conversation = conversationHistoryService.createConversation('빈 대화');
      const context = conversationHistoryService.getContextForLLM(conversation.id);
      expect(context).toEqual([]);
    });

    it('maxMessages 1이면 마지막 메시지만 반환해야 함', () => {
      const conversation = conversationHistoryService.createConversation('대화');
      conversationHistoryService.addMessage(conversation.id, { role: 'user', content: '첫 메시지' });
      conversationHistoryService.addMessage(conversation.id, { role: 'assistant', content: '마지막 메시지' });

      const context = conversationHistoryService.getContextForLLM(conversation.id, 1);
      expect(context.length).toBe(1);
      expect(context[0].content).toBe('마지막 메시지');
    });

    it('assistant 메시지의 pipelineExtras가 컨텍스트 턴에 포함된다', () => {
      const conversation = conversationHistoryService.createConversation('대화');
      conversationHistoryService.addMessage(conversation.id, { role: 'user', content: 'q' });
      conversationHistoryService.addMessage(conversation.id, {
        role: 'assistant',
        content: 'a',
        pipelineExtras: { qaPipelineTraceId: 'nb-trace' },
      });
      const context = conversationHistoryService.getContextForLLM(conversation.id);
      expect(context[1].pipelineExtras?.qaPipelineTraceId).toBe('nb-trace');
    });
  });
  ...
  describe('localStorage 통합', () => {
    it('대화가 localStorage에 저장되어야 함', () => {
      ...
      const stored = localStorage.getItem(CONVERSATION_HISTORY_STORAGE_KEY);
    ...
    it('localStorage 저장 실패 시 에러 로그가 남는다', () => {
      const setSpy = jest.spyOn(localStorageMock, 'setItem').mockImplementation(() => {
        throw new Error('setItem failed');
      });
      try {
        conversationHistoryService.createConversation('저장 실패 테스트');
  ...
  describe('로드 시 메시지 정리 (격리 모듈)', () => {
    it('assistant 생성 플레이스홀더는 로드 시 미완료 안내로 치환된다', () => {
      localStorageMock.clear();
      const payload = [
        {
          id: 'conv-ph-isolated',
      ...
      localStorage.setItem(CONVERSATION_HISTORY_STORAGE_KEY, JSON.stringify(payload));

      jest.isolateModules(() => {
        // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires
        const { default: svc } = require('../conversationHistoryService');
        const c = svc.getConversation('conv-ph-isolated');
        expect(c?.messages[0].content).toContain('미완료');
      });
    });

    it('저장 값이 배열이 아니면 로드 시 오류 로그 없이 대화가 비어 있다', () => {
      localStorageMock.clear();
      localStorage.setItem(CONVERSATION_HISTORY_STORAGE_KEY, '{}');
      jest.isolateModules(() => {
        // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires
        const { errorLogger } = require('../../utils/errorLogger');
        jest.mocked(errorLogger.error).mockClear();
        // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires
        const { default: svc } = require('../conversationHistoryService');
        expect(svc.getConversations()).toEqual([]);
        expect(errorLogger.error).not.toHaveBeenCalled();
      });
    });

    it('messages가 배열이 아니면 빈 메시지 목록으로 로드된다', () => {
      localStorageMock.clear();
      const payload = [
        {
          id: 'conv-bad-messages',
          title: 't',
          messages: null,
          createdAt: '2020-01-01T00:00:00.000Z',
          updatedAt: '2020-01-01T00:00:00.000Z',
      ...
      jest.isolateModules(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires
        const { default: svc } = require('../conversationHistoryService');
        const c = svc.getConversation('conv-bad-messages');
