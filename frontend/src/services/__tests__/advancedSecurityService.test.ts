...
jest.mock('axios', () => {
  ...
  const mockInstance = { get: mockGet, post: mockPost, create: jest.fn() };
  (mockInstance as unknown as Record<string, unknown>).create = jest.fn(() => mockInstance);
  return {
    default: {
      get: mockGet,
      post: mockPost,
...
describe('AdvancedSecurityService', () => {
    ...
    describe('getSecurityThreats', () => {
        it('should fetch security threats successfully', async () => {
            ...
            expect(mockedAxios.get).toHaveBeenCalledWith('/security/threats?');
        });
    ...
    describe('runSecurityScan', () => {
        it('should run security scan successfully', async () => {
            ...
            expect(mockedAxios.post).toHaveBeenCalledWith(ADVANCED_SECURITY_SCAN_PATH, null, {
    ...
    describe('getAuditLogs', () => {
        ...
        it('should filter by user_id when provided', async () => {
            ...
            expect(mockedAxios.get).toHaveBeenCalledWith(
                `${ADVANCED_SECURITY_AUDIT_LOGS_PATH}?${API_QUERY_PARAM_USER_ID}=user_1&${API_QUERY_PARAM_LIMIT}=100`,
            );
        });
    });
});
