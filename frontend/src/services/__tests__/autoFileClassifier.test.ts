...
describe('AutoFileClassifier', () => {
  ...
  beforeEach(() => {
    ...
    // 기본 모킹
    jest.mocked(clientFileProcessor).processFile = jest.fn().mockResolvedValue({
      fileType: 'text',
    ...
    jest.mocked(mediaAnalysisService).analyzeMedia = jest.fn().mockResolvedValue({
      fileType: 'image',
      metadata: {},
  ...
  describe('파일 분류', () => {
    ...
    it('분류 실패 시 기본 분류 반환', async () => {
      jest.mocked(clientFileProcessor.processFile).mockRejectedValue(new Error('Processing failed'));
