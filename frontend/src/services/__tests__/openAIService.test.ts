...
import { OPENAI_OFFICIAL_API_V1_BASE_URL } from '../../config/api';
...
describe('OpenAIService', () => {
  ...
  describe('대화', () => {
    ...
    it('기본 대화 요청', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        openAIChatCompletionsUrl(OPENAI_OFFICIAL_API_V1_BASE_URL),
  ...
  describe('환경 변수 설정', () => {
    it('기본 URL 사용', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining(OPENAI_OFFICIAL_API_V1_BASE_URL),
        expect.any(Object)
      );
    });
    ...
    it('커스텀 URL 사용', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        openAIChatCompletionsUrl('https://custom-api.com/v1'),
        expect.any(Object)
      );
    });
