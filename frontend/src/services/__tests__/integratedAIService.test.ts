...
import integratedAIService, { AIRequest } from '../integratedAIService';
...
jest.mock('../aiCacheManager', () => {
  ...
  const mockGetStats = jest.fn().mockReturnValue({ total_entries: 0, hit_rate: 0, miss_rate: 1, eviction_count: 0, memory_usage: 0, oldest_entry: null, newest_entry: null, most_accessed_key: null, cache_efficiency: 0, total_size: 0 });
  const mockOptimize = jest.fn();
  const mockDeleteByTag = jest.fn().mockReturnValue(0);
  return {
    __esModule: true,
    default: {
      start: mockStart,
