...
describe('conversationMemoryService', () => {
  beforeEach(() => {
    conversationMemoryService.exportLearningData();
  ...
  describe('getPersonalizedSuggestions', () => {
    ...
    it('성향 키워드가 있으면 성향 분석 관련 제안이 포함될 수 있다', () => {
      conversationMemoryService.saveConversation({
        question: '참여자들의 성향을 분석해줘',
        response: 'r',
        processingTime: 50,
        context: {},
      });
      const suggestions = conversationMemoryService.getPersonalizedSuggestions();
      expect(suggestions.some((s) => s.includes('성향'))).toBe(true);
    });

    it('자세히 요청이 있으면 심층 분석 제안이 포함될 수 있다', () => {
      conversationMemoryService.saveConversation({
        question: '자세히 설명해줘',
        response: 'r',
        processingTime: 50,
      ...
      const suggestions = conversationMemoryService.getPersonalizedSuggestions();
      expect(suggestions.some((s) => s.includes('심층'))).toBe(true);
    });
  });

  describe('기록 제한', () => {
    it('maxRecords 초과 시 가장 오래된 기록부터 잘린다', () => {
      const svc = conversationMemoryService as unknown as { maxRecords: number };
      const prev = svc.maxRecords;
      try {
        svc.maxRecords = 4;
        for (let i = 0; i < 6; i += 1) {
          conversationMemoryService.saveConversation({
        ...
        const history = conversationMemoryService.getConversationHistory(100);
        expect(history).toHaveLength(4);
        const questions = history.map((c) => c.question);
        expect(questions).toContain('q5');
        expect(questions).not.toContain('q0');
      } finally {
        svc.maxRecords = prev;
      }
    });
  ...
  describe('getPerformanceStats', () => {
    ...
    it('analysisType 분포를 포함해야 함', () => {
      ...
      const stats = conversationMemoryService.getPerformanceStats();
      /* eslint-disable jest/no-conditional-expect -- stats may be null */
      if (stats) {
    ...
    it('응답 시간 개선도는 최근 10건과 그 이전 10건 처리 시간을 비교한다', () => {
      ...
      const stats = conversationMemoryService.getPerformanceStats();
      expect(stats?.responseTimeImprovement).toBe(50);
    });
  });
