...
describe('VoiceService', () => {
  ...
  describe('음성 인식', () => {
    ...
    it('음성 인식 결과 처리', () => {
      ...
      // Mock result event (Speech API: results[i] is array-like with [0] and .isFinal)
      const resultRow = [{ transcript: '안녕하세요', confidence: 0.9 }];
      (resultRow as unknown as { isFinal: boolean }).isFinal = true;
      const mockEvent = {
        resultIndex: 0,
        results: [resultRow],
      };
    ...
    it('임시 음성 인식 결과 처리', () => {
      ...
      const resultRow = [{ transcript: '안녕', confidence: 0.8 }];
      (resultRow as unknown as { isFinal: boolean }).isFinal = false;
      const mockEvent = {
        resultIndex: 0,
        results: [resultRow],
      };
  ...
  describe('에지 케이스', () => {
    it('음성 인식 미지원 브라우저', () => {
      ...
      win.webkitSpeechRecognition = originalWebkitSpeechRecognition;
    });
