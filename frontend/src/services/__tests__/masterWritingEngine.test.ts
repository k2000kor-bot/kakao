...
describe('MasterWritingEngine', () => {
    ...
    beforeEach(() => {
        ...
        // 모킹된 엔진들의 반환값 설정
        jest.mocked(politicalWritingEngine).generatePoliticalWriting = jest.fn().mockResolvedValue({
            generatedText: '정치적 글쓰기 텍스트',
            keyArguments: ['정치적 논점 1', '정치적 논점 2']
        });

        jest.mocked(generationWritingEngine).generateGenerationWriting = jest.fn().mockResolvedValue({
            generatedText: '세대별 글쓰기 텍스트',
            generationalCharacteristics: ['세대 특성 1', '세대 특성 2']
        });

        jest.mocked(stanceWritingEngine).generateStanceWriting = jest.fn().mockResolvedValue({
    ...
    describe('generateMasterWriting', () => {
        ...
        it('에러 발생 시 적절히 처리해야 함', async () => {
            jest.mocked(politicalWritingEngine.generatePoliticalWriting).mockRejectedValueOnce(
