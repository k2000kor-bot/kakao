...
import { notificationService, NOTIFICATION_SETTINGS_STORAGE_KEY } from '../notificationService';
...
beforeAll(() => {
  // Notification API 모킹
  (global as unknown as Record<string, unknown>).Notification = jest.fn().mockImplementation((title: string, options?: Record<string, unknown>) => ({
    title,
    ...options,
    onclick: null,
    close: jest.fn(),
  }));
  Object.assign(global as unknown as Record<string, unknown>, { Notification: { ...mockNotification, permission: 'granted' } });

  // AudioContext 모킹
  (global as unknown as Record<string, unknown>).AudioContext = jest.fn(() => mockAudioContext);
  (global as unknown as Record<string, unknown>).webkitAudioContext = jest.fn(() => mockAudioContext);
