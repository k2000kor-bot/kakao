...
describe('MediaKnowledgeAPI', () => {
    ...
    describe('createProject', () => {
        it('프로젝트를 생성해야 함', async () => {
            ...
            expect(result).toEqual(mockResponse);
            expect(global.fetch).toHaveBeenCalledWith(
                joinApiHealthCheckUrl(FALLBACK_API_ORIGIN, `${API_PROJECTS_LIST_PATH}`),
                expect.objectContaining({
                    method: 'POST',
    ...
    describe('uploadMediaFile', () => {
        it('미디어 파일을 업로드해야 함', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(
                joinApiHealthCheckUrl(FALLBACK_API_ORIGIN, `${UPLOAD_MEDIA_PATH}`),
                expect.objectContaining({
                    method: 'POST',
    ...
    describe('analyzeFile', () => {
        it('파일을 분석해야 함', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(
                joinApiHealthCheckUrl(
                    FALLBACK_API_ORIGIN,
                    `${ANALYZE_FILE_PATH_PREFIX}/${encodeURIComponent(fileId)}`,
                ),
                expect.objectContaining({
                    method: 'POST'
                })
