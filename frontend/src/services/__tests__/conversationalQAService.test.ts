...
describe('ConversationalQAService', () => {
    ...
    describe('질문 처리', () => {
        it('기본 질문 처리', async () => {
            jest.mocked(unifiedAPI.conversationalQA).mockResolvedValue({
                success: true,
                data: {
                    answer: '테스트 답변입니다.',
                    confidence: 0.9,
        ...
        it('후속 질문 처리', async () => {
            // 첫 번째 질문
            jest.mocked(unifiedAPI.conversationalQA).mockResolvedValueOnce({
                success: true,
                data: {
                    answer: '첫 번째 답변',
                    confidence: 0.9,
        ...
        it('API 실패 시 폴백 응답', async () => {
            jest.mocked(unifiedAPI.conversationalQA).mockRejectedValue(new Error('API 오류'));
