...
describe('PerformanceMonitoringService', () => {
  ...
  describe('API 호출 모니터링', () => {
    it('성공적인 API 호출 모니터링', async () => {
      ...
      expect(apiPerf[apiPerf.length - 1].endpoint).toBe(API_SMOKE_TEST_PATH);
    ...
    it('실패한 API 호출 모니터링', async () => {
      ...
      await expect(
        service.monitorApiCall(API_SMOKE_TEST_PATH, 'GET', mockApiCall)
    ...
    it('API 호출 응답 시간 측정', async () => {
      ...
      await service.monitorApiCall(API_SMOKE_TEST_PATH, 'POST', mockApiCall);
  ...
  describe('서비스 정리', () => {
    it('서비스 정리', () => {
      const observer = (service as unknown as { performanceObserver: unknown }).performanceObserver;
      if (observer) {
