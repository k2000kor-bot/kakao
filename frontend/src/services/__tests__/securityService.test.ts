...
import { API_QUERY_PARAM_LIMIT, AUTH_RESET_PASSWORD_PATH, SECURITY_EVENTS_PATH } from '../../config/api';
...
describe('SecurityService', () => {
  ...
  beforeEach(() => {
    ...
    Object.assign(mockedAxios, { isAxiosError: jest.fn(() => false) });
  ...
  describe('로그아웃', () => {
    it('로그아웃 실행', async () => {
      ...
      expect(localStorageMock.removeItem).toHaveBeenCalledWith(CURRENT_USER_STORAGE_KEY);
    });
  });
  ...
  describe('토큰 갱신', () => {
    it('토큰 갱신 성공', async () => {
      ...
      // 토큰 설정
      (service as unknown as { authToken: string | null }).authToken = oldToken;
  ...
  describe('권한 관리', () => {
    ...
    it('관리자는 모든 권한', async () => {
      ...
      (service as unknown as { currentUser: unknown }).currentUser = adminUser;
  ...
  describe('보안 이벤트', () => {
    ...
    it('보안 이벤트 조회', async () => {
      ...
      expect(mockAxiosInstance.get).toHaveBeenCalledWith(SECURITY_EVENTS_PATH, {
        params: { [API_QUERY_PARAM_LIMIT]: 100 },
      });
    });
  ...
  describe('저장된 인증 정보 로드', () => {
    it('localStorage에서 인증 정보 로드', () => {
      ...
      localStorageMock.setItem(AUTH_TOKEN_STORAGE_KEY, JSON.stringify(mockToken));
      localStorageMock.setItem(CURRENT_USER_STORAGE_KEY, JSON.stringify(mockUser));
