...
class MockCanvasRenderingContext2D {
  ...
  drawImage(_image: HTMLImageElement, _dx: number, _dy: number): void {
    // 이미지 그리기 시뮬레이션
  }
...
(global as unknown as Record<string, unknown>).HTMLCanvasElement = MockCanvas;
...
class MockFileReader {
  ...
  onload: ((event: Event & { target?: { result?: unknown } }) => void) | null = null;
...
describe('ImageAnalysisService', () => {
  ...
  describe('이미지 분석', () => {
    ...
    it.skip('색상 분석 포함', async () => {
      // jsdom canvas mock은 빈 getImageData 반환 → dominant/palette/brightness 등 NaN·빈 배열
      const file = new File(['test image'], 'test.jpg', { type: 'image/jpeg' });
  ...
  describe('에러 처리', () => {
    it('잘못된 파일 처리', async () => {
      ...
      (global as unknown as Record<string, unknown>).FileReader = class {
        onerror: ((event: Event | Error) => void) | null = null;
        readAsDataURL() {
          setTimeout(() => {
            if (this.onerror) {
  ...
  describe('이미지 품질 메트릭', () => {
    it.skip('선명도 계산', async () => {
      // jsdom canvas mock → getImageData 빈 데이터 → sharpness NaN
      const file = new File(['test image'], 'test.jpg', { type: 'image/jpeg' });
    ...
    it.skip('노이즈 계산', async () => {
      // jsdom canvas mock → getImageData 빈 데이터 → noise NaN
      const file = new File(['test image'], 'test.jpg', { type: 'image/jpeg' });
  ...
  describe('색상 분석 상세', () => {
    it.skip('주요 색상 팔레트 반환', async () => {
      // jsdom canvas mock → palette 빈 배열
      const file = new File(['test image'], 'test.jpg', { type: 'image/jpeg' });
    ...
    it.skip('밝기 값 범위', async () => {
      // jsdom canvas mock → brightness NaN
      const file = new File(['test image'], 'test.jpg', { type: 'image/jpeg' });
    ...
    it.skip('대비 값 범위', async () => {
      // jsdom canvas mock → contrast NaN
      const file = new File(['test image'], 'test.jpg', { type: 'image/jpeg' });
    ...
    it.skip('채도 값 범위', async () => {
      // jsdom canvas mock → saturation NaN
      const file = new File(['test image'], 'test.jpg', { type: 'image/jpeg' });
