...
import { API_BASE_URL, API_PERSISTENT_SESSIONS_PATH, joinApiHealthCheckUrl } from '../../config/api';
...
describe('ChatGPTProjectService', () => {
  ...
  beforeEach(() => {
    ...
    service.baseUrl = API_BASE_URL;
    // errorHandler.safeApiCall 모킹 초기화
    jest.mocked(errorHandlerModule.errorHandler.safeApiCall).mockClear();
  });
  ...
  describe('createProject', () => {
    it('프로젝트를 성공적으로 생성해야 함', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        joinApiHealthCheckUrl(API_BASE_URL, API_PERSISTENT_SESSIONS_PATH),
  ...
  describe('getProjects', () => {
    it('프로젝트 목록을 성공적으로 조회해야 함', async () => {
      ...
      jest.mocked(errorHandlerModule.errorHandler.safeApiCall).mockResolvedValueOnce({
        success: true,
        data: mockProjects,
      });
    ...
    it('safeApiCall이 null을 반환하면 빈 배열을 반환해야 함', async () => {
      ...
      jest.mocked(errorHandlerModule.errorHandler.safeApiCall).mockResolvedValueOnce(null);
