...
describe('writingStyleService', () => {
  ...
  describe('getStyleInstruction', () => {
    it('존재하는 스타일은 톤·구조·특징을 포함한 지시문을 반환해야 함', () => {
      const instruction = writingStyleService.getStyleInstruction('novelist');
      expect(typeof instruction).toBe('string');
      expect(instruction.length).toBeGreaterThan(0);
      expect(instruction).toContain('소설가');
      expect(instruction).toContain('톤');
      expect(instruction).toContain('구조');
    });
    it('시인 스타일은 시인·톤을 포함한 지시문을 반환해야 함', () => {
      const instruction = writingStyleService.getStyleInstruction('poet');
      expect(typeof instruction).toBe('string');
      expect(instruction.length).toBeGreaterThan(0);
      expect(instruction).toContain('시인');
      expect(instruction).toContain('톤');
    });
    it('존재하지 않는 스타일은 빈 문자열을 반환해야 함', () => {
      const instruction = writingStyleService.getStyleInstruction('nonexistent');
      expect(instruction).toBe('');
    });
    it('빈 문자열 styleId는 빈 문자열을 반환해야 함', () => {
      const instruction = writingStyleService.getStyleInstruction('');
      expect(instruction).toBe('');
    });
  });
