...
(global as unknown as Record<string, unknown>).Notification = mockNotification;
...
describe('MobileOptimizationService', () => {
  ...
  describe('이미지 최적화', () => {
    ...
    it('다양한 크기로 이미지 최적화 URL 생성', () => {
      const url = 'https://cdn.example.com/photo.png';
      const small = service.optimizeImage(url, 100, 100);
      const large = service.optimizeImage(url, 800, 600);
  ...
  describe('오프라인 모드', () => {
    it('오프라인 모드 활성화', () => {
      ...
      expect(() => service.enableOfflineMode()).not.toThrow();
    });
  });
  ...
  describe('PWA 설치', () => {
    it('설치 프롬프트 표시', async () => {
      ...
      (window as unknown as Record<string, unknown>).deferredPrompt = mockPrompt;
      (window as unknown as Record<string, unknown>).beforeinstallprompt = true;
    ...
    it('설치 프롬프트 없을 때 처리', async () => {
      delete (window as unknown as Record<string, unknown>).deferredPrompt;
      delete (window as unknown as Record<string, unknown>).beforeinstallprompt;
