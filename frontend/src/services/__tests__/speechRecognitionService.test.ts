...
describe('SpeechRecognitionService', () => {
  ...
  interface MockRecognitionShape { lang: string; continuous: boolean; interimResults: boolean; maxAlternatives: number; start: jest.Mock; stop: jest.Mock; abort: jest.Mock; onstart: (() => void) | null; onend: (() => void) | null; onerror: ((e: unknown) => void) | null; onresult: ((e: unknown) => void) | null }
  let mockRecognition: MockRecognitionShape;
  ...
  describe('마이크 권한', () => {
    ...
    it('마이크 권한 확인 실패 처리', async () => {
      jest.mocked(navigator.mediaDevices.getUserMedia).mockRejectedValueOnce(
  ...
  describe('음성 인식 결과 처리', () => {
    it('인식 결과 이벤트 테스트', () => {
      ...
      const events: SpeechRecognitionEvents = { onResult };
      expect(events).toHaveProperty('onResult');
  ...
  describe('에러 처리', () => {
    it('에러 이벤트 핸들러 설정', () => {
      const onError = jest.fn();
      const events: SpeechRecognitionEvents = { onError };
    ...
    it('음성 없음 에러 처리', () => {
      ...
      const events: SpeechRecognitionEvents = { onNoSpeech };
      expect(events).toHaveProperty('onNoSpeech');
  ...
  describe('음성 활동 감지', () => {
    it('음성 활동 이벤트 핸들러 설정', () => {
      ...
      const events: SpeechRecognitionEvents = { onVoiceActivity };
      expect(events).toHaveProperty('onVoiceActivity');
