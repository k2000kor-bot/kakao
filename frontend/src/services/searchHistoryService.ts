...
class SearchHistoryService {
  private config: SearchHistoryConfig = {
    ...
    savedSearchesKey: SAVED_SEARCHES_STORAGE_KEY,
  };
  ...
  getAutocompleteSuggestions(input: string, limit: number = 5): string[] {
    const trimmedInput = coerceTrimmedString(input || '', '').toLowerCase();
  ...
  shareSearch(id: string): string {
    ...
    if (search) {
      ...
      try {
        ...
        localStorage.setItem(SHARED_SEARCHES_STORAGE_KEY, JSON.stringify(sharedSearches));
      } catch (error) {
  ...
  getSharedSearches(): SavedSearch[] {
    try {
      const stored = localStorage.getItem(SHARED_SEARCHES_STORAGE_KEY);
      if (!stored) return [];
