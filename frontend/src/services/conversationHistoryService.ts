...
export interface ConversationMessage {
  ...
  content: string;
  timestamp: string;
  /** 통합 Q→A·merge 상속용 — `getContextForLLM` 턴에 선택 포함 */
...
class ConversationHistoryService {
  ...
  private readonly storageKey = CONVERSATION_HISTORY_STORAGE_KEY;
  ...
  /**
   * 컨텍스트 추출 (LLM에 전달할 형식). `pipelineExtras`가 있으면 턴에 실어 백엔드 merge·상속에 사용 가능.
   */
  getContextForLLM(
    conversationId: string,
    maxMessages?: number
  ): Array<{ role: string; content: string; pipelineExtras?: PipelineMessageExtras }> {
    const conversation = this.conversations.get(conversationId);
    if (!conversation) {
      return [];
    }
    ...
    return messages.map((msg) => {
      const row: { role: string; content: string; pipelineExtras?: PipelineMessageExtras } = {
        role: msg.role,
        content: msg.content,
      };
      if (
        ...
        hasPipelineExtras(msg.pipelineExtras)
      ) {
        row.pipelineExtras = msg.pipelineExtras;
      }
      return row;
    });
  }
  ...
  private loadConversations(): void {
    try {
      ...
      if (stored) {
        const conversationsArray = JSON.parse(stored) as unknown;
        if (!Array.isArray(conversationsArray)) {
          return;
        }
        conversationsArray.forEach((raw: ConversationContext) => {
          if (!raw?.id) {
            return;
          }
          const messages = Array.isArray(raw.messages) ? raw.messages : [];
          const sanitized: ConversationContext = {
            ...raw,
            messages: messages.map((msg) =>
              msg.role === 'assistant' && isAssistantGenerationPlaceholder(msg.content)
                ? { ...msg, content: STORED_ASSISTANT_INCOMPLETE_NOTICE }
                : msg
            ),
          };
          this.conversations.set(raw.id, sanitized);
        });
      }
    } catch (error) {
...
export { CONVERSATION_HISTORY_STORAGE_KEY } from './conversationHistoryStorageKeys';
