 ...
 * API 주소는 config/api(5002)와 동일하게 사용
 */
...
import {
  ...
  API_V7_PREDICT_MESSAGE_QUALITY_PATH,
  API_V7_PREDICT_SUMMARY_PATH,
  API_V7_PREDICT_SYSTEM_PERFORMANCE_PATH,
  API_V7_PREDICT_USER_ACTIVITY_PATH,
...
class AdvancedAPIService {
  ...
  constructor() {
    this.api = axios.create({
      baseURL: resolveAxiosHttpOriginBaseUrl((API_BASE_URL || FALLBACK_API_ORIGIN).trim()),
      timeout: 30000,
      headers: {
  ...
  async startVoiceRecognition(
    ...
    try {
      ...
      const message = errorHandler.handleAPIError(error as Error, API_V7_VOICE_START_RECOGNITION_PATH);
  ...
  async getVoiceRecognitionResults(
    ...
    try {
      const params = sessionId ? { [API_QUERY_PARAM_SESSION_ID]: sessionId } : {};
