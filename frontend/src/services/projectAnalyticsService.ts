...
class ProjectAnalyticsService {
     ...
     * 활동 트렌드 계산
     */
    private calculateActivityTrend(messages: unknown[]): Array<{ date: string; count: number }> {
        ...
        messages.forEach(msg => {
            const m = msg as { timestamp?: string | Date };
            if (m.timestamp) {
     ...
     * 메시지 증가율 계산
     */
    private calculateMessageGrowth(messages: unknown[]): number {
        const now = new Date();
        const thisMonth = messages.filter(msg => {
            const m = msg as { timestamp?: string | Date };
            const msgDate = new Date(m.timestamp ?? 0);
     ...
     * 평균 응답 시간 계산
     */
    private calculateAverageResponseTime(_messages: unknown[]): number {
     ...
     * 가장 활발한 시간대 계산
     */
    private calculateMostActiveTime(messages: unknown[]): string {
     ...
     * 주요 키워드 추출
     */
    private extractTopKeywords(messages: unknown[], limit: number = 10): Array<{ keyword: string; count: number }> {
     ...
     * 완료율 계산
     */
    private calculateCompletionRate(sessions: unknown[]): number {
    ...
    generateInsights(analytics: ProjectAnalytics): string[] {
        ...
        if (analytics.topKeywords.length > 0) {
            insights.push(`🔍 주요 키워드: ${analytics.topKeywords.map(k => k.keyword).join(', ')}`);
        }
