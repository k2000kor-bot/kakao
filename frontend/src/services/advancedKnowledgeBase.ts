...
class AdvancedKnowledgeBase {
     ...
     * 🎯 문제 상황별 해결책 제안
     */
    getSolutionsForProblem(problem: string, context: unknown): Array<{
        solution: string;
        ...
        if (problemLower.includes('느려') || problemLower.includes('성능') || problemLower.includes('속도')) {
            ...
            if ((context as { technologies?: string[] })?.technologies?.includes('React')) {
                solutions.push({
    ...
    getPersonalizedLearningPath(
            ...
            'backend-developer': {
                phases: [
                    ...
                    },
                    {
                        ...
                        projects: ['마이크로서비스 아키텍처', '실시간 대화 서버'],
        ...
        type LearningPathItem = { phase: string; duration: string; topics: string[]; projects: string[]; resources: string[] };
        type LearningPathEntry = { phases: LearningPathItem[]; totalDuration: string; difficulty: string };
        const defaultPath = { path: [] as LearningPathItem[], totalDuration: '개인 맞춤 계획 필요', difficulty: 'varies' };
        const keyed = (learningPaths as Record<string, LearningPathEntry>)[targetRole];
        return keyed ? { path: keyed.phases, totalDuration: keyed.totalDuration, difficulty: keyed.difficulty } : defaultPath;
    }
