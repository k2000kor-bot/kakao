...
class ConstructionCompanyService {
    ...
    async getCompanies(searchParams?: {
        ...
        try {
            const response = await axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, API_CONSTRUCTION_COMPANIES_PATH), {
                params: searchParams,
            });
    ...
    async getCompany(companyId: string): Promise<ConstructionCompany | null> {
        try {
            const response = await axios.get(
                joinApiHealthCheckUrl(
                    CONFIG_API_ORIGIN,
                    `${API_CONSTRUCTION_COMPANIES_PATH}/${encodeURIComponent(companyId)}`,
                ),
            );
    ...
    async getDefectIssues(companyId?: string, projectId?: string): Promise<DefectIssue[]> {
        try {
            const response = await axios.get(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, API_CONSTRUCTION_DEFECT_ISSUES_PATH), {
                params: {
                    ...(companyId && { [API_QUERY_PARAM_COMPANY_ID]: companyId }),
                    ...(projectId && { [API_QUERY_PARAM_PROJECT_ID]: projectId }),
                },
            });
    ...
    async createDefectIssue(issue: Omit<DefectIssue, 'id' | 'reported_date' | 'status'>): Promise<DefectIssue> {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, API_CONSTRUCTION_DEFECT_ISSUES_PATH), issue);
    ...
    async generateResponsePlan(issueId: string, companyId: string): Promise<ResponsePlan> {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, API_CONSTRUCTION_RESPONSE_PLANS_PATH), {
                [API_QUERY_PARAM_ISSUE_ID]: issueId,
                [API_QUERY_PARAM_COMPANY_ID]: companyId,
            });
    ...
    async analyzeSelectionCriteria(
        ...
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, API_CONSTRUCTION_SELECTION_ANALYSIS_PATH), {
                [API_QUERY_PARAM_COMPANY_IDS]: companyIds,
                ...(criteria && { criteria }),
            });
    ...
    async compareCompanies(companyIds: string[]): Promise<CompanyComparison> {
        try {
            const response = await axios.post(joinApiHealthCheckUrl(CONFIG_API_ORIGIN, API_CONSTRUCTION_COMPARE_PATH), {
                [API_QUERY_PARAM_COMPANY_IDS]: companyIds,
            });
        ...
        return [
            {
                ...
                contact_info: {
                    ...
                    website: DEMO_SAMSUNG_CORP_WEBSITE_URL,
                },
            },
            {
                id: 'company-2',
                ...
                contact_info: {
                    ...
                    website: DEMO_HYUNDAI_CORP_WEBSITE_URL,
                },
            },
            {
                id: 'company-3',
    ...
    private getSampleDefectIssues(): DefectIssue[] {
        return [
            {
                id: 'issue-1',
                [API_QUERY_PARAM_COMPANY_ID]: 'company-1',
                company_name: '삼성물산 건설부문',
                [API_QUERY_PARAM_PROJECT_ID]: 'project-1',
            ...
            {
                id: 'issue-2',
                [API_QUERY_PARAM_COMPANY_ID]: 'company-2',
                company_name: '현대건설',
                [API_QUERY_PARAM_PROJECT_ID]: 'project-2',
    ...
    private getSampleSelectionAnalysis(companyId: string): SelectionCriteriaAnalysis {
        ...
        return {
            [API_QUERY_PARAM_COMPANY_ID]: company.id,
    ...
    private getSampleComparison(companyIds: string[]): CompanyComparison {
        ...
        return {
            companies: selectedCompanies.map((c) => ({
                [API_QUERY_PARAM_COMPANY_ID]: c.id,
                company_name: c.name,
                overall_score: 7.8,
                criteria_scores: {
