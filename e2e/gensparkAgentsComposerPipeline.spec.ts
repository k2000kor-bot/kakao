...
import { buildComposerStreamSseBody } from './helpers/composerChatStub';
...
describeAgentsComposer('/agents 에이전트 세션 — 입력창 하단 생성 단계 UI', () => {
  ...
  test('구조화 질문 완료 후 Oversight Council 패널이 과업 메타에 표시된다', async ({ page }) => {
    ...
    const sseBody = buildComposerStreamSseBody('E2E Council 스텁 응답');
    ...
    await expect(page.getByText('E2E Council 스텁 응답')).toBeVisible({ timeout: 20_000 });
