...
if [ -d ".venv" ]; then
    ...
    echo "❌ Python 가상환경이 없습니다. 먼저 가상환경을 생성해주세요."
    exit 1
fi
...
python chatgpt_unified_system.py &
EXISTING_PID=$!
...
python simple_test_server.py 8007 &
MARKET_PID=$!
echo "   시장 분석 엔진 PID: $MARKET_PID"

python simple_test_server.py 8008 &
DREAM_PID=$!
...
python simple_test_server.py 8009 &
PERFORMANCE_PID=$!
echo "   성능 최적화 시스템 PID: $PERFORMANCE_PID"

python simple_test_server.py 8010 &
SCALABILITY_PID=$!
...
python simple_test_server.py 8011 &
ADVANCED_AI_PID=$!
...
python simple_test_server.py 8012 &
PLANNING_PID=$!
