...
const useChatEnhancements = (config: ChatEnhancementConfig = {}) => {
  // 타입 가드 함수
  const isValidTypingIndicator = (indicator: unknown): indicator is TypingIndicator => {
    return (
      typeof indicator === 'object' &&
      indicator !== null &&
      'userId' in indicator &&
      'userName' in indicator &&
      'timestamp' in indicator &&
      typeof (indicator as TypingIndicator).userId === 'string' &&
      typeof (indicator as TypingIndicator).userName === 'string' &&
      (indicator as TypingIndicator).timestamp instanceof Date
    );
  };

  const isValidReadReceipt = (receipt: unknown): receipt is ReadReceipt => {
    return (
      ...
      'userId' in receipt &&
      'readAt' in receipt &&
      typeof (receipt as ReadReceipt).messageId === 'string' &&
      typeof (receipt as ReadReceipt).userId === 'string' &&
      (receipt as ReadReceipt).readAt instanceof Date
    );
  };

  const isValidMessageReaction = (reaction: unknown): reaction is MessageReaction => {
    return (
      typeof reaction === 'object' &&
      reaction !== null &&
      'messageId' in reaction &&
      'userId' in reaction &&
      'reaction' in reaction &&
      'timestamp' in reaction &&
      typeof (reaction as MessageReaction).messageId === 'string' &&
      typeof (reaction as MessageReaction).userId === 'string' &&
      typeof (reaction as MessageReaction).reaction === 'string' &&
      (reaction as MessageReaction).timestamp instanceof Date
    );
  };
  const {
    ...
    const handleSyncEvent = (event: any) => {
      if (event.type === 'typing') {
        const typingIndicator: TypingIndicator = {
          ...
          userName: (event.data as any)?.userName || '사용자',
