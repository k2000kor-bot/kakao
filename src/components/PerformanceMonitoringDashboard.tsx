...
const PerformanceMonitoringDashboard: React.FC<PerformanceMonitoringDashboardProps> = ({
  ...
  const fetchMetrics = useCallback(async () => {
    try {
      ...
      // 백엔드 API에서 시스템 상태 조회
      const response = await fetch('http://localhost:8000/api/v7/status');
      if (response.ok) {
        const data = await response.json();
        if (data.system_status) {
          setMetrics({
            cpu: (data.system_status.system_metrics?.cpu_usage || 0) * 100,
            memory: (data.system_status.system_metrics?.memory_usage || 0) * 100,
            disk: (data.system_status.system_metrics?.disk_usage || 0) * 100,
            responseTime: 0,
      ...
      setError(err.message || '성능 메트릭 조회 중 오류가 발생했습니다.');
    } finally {
      stopLoading();
    }
  ...
  const getMetricColor = (value: number, threshold: number = 80): string => {
    if (value >= threshold) return '#dc3545'; // 위험 (빨강)
    if (value >= threshold * 0.7) return '#ffc107'; // 경고 (노랑)
    return '#28a745'; // 정상 (초록)
  };
