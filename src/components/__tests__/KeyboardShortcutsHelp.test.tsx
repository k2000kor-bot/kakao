...
describe('KeyboardShortcutsHelp', () => {
  ...
  it('카테고리 버튼 클릭 시 필터링되어야 함', () => {
    ...
    if (navigationButton) {
      ...
      // 네비게이션 카테고리의 단축키만 표시되어야 함
      expect(screen.getByText('새 채팅 시작')).toBeInTheDocument();
  ...
  it('검색 카테고리 필터링이 작동해야 함', () => {
    ...
    if (searchButton) {
      ...
      expect(screen.getByText('검색 결과 이동')).toBeInTheDocument();
      expect(screen.getByText('검색 결과 선택')).toBeInTheDocument();
    }
  });
  ...
  it('일반 카테고리 필터링이 작동해야 함', () => {
    ...
    if (generalButton) {
      ...
      expect(screen.getByText('단축키 도움말')).toBeInTheDocument();
      expect(screen.getByText('다크 모드 토글')).toBeInTheDocument();
    }
  });
  ...
  it('푸터 메시지가 표시되어야 함', () => {
    ...
    expect(screen.getByText(/팁: 대부분의 단축키는 입력창에 포커스가 있을 때 작동합니다/)).toBeInTheDocument();
  });
});
