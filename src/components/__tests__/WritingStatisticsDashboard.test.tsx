...
const mockAnalyzeQuality = jest.fn();
...
describe('WritingStatisticsDashboard', () => {
  ...
  beforeEach(() => {
    ...
    (writingQualityAnalyzer.analyzeQuality as jest.Mock).mockReturnValue(mockAnalysis);
  });
  ...
  describe('localStorage 통계 로드', () => {
    it('localStorage에서 히스토리를 로드하여 통계를 계산해야 함', async () => {
      localStorageMock.setItem('writingHistory', JSON.stringify(mockHistoryItems));
  ...
  describe('내용 변경 처리', () => {
    it('내용이 변경되면 통계가 업데이트되어야 함', async () => {
      ...
      const callCountBefore = (writingQualityAnalyzer.analyzeQuality as jest.Mock).mock.calls.length;

      rerender(<WritingStatisticsDashboard content="변경된 내용" />);

      await waitFor(() => {
        expect((writingQualityAnalyzer.analyzeQuality as jest.Mock).mock.calls.length).toBeGreaterThan(callCountBefore);
      });
    });
  ...
  describe('에러 처리', () => {
    it('localStorage 파싱 에러가 발생해도 정상적으로 작동해야 함', () => {
      localStorageMock.setItem('writingHistory', 'invalid json');
  ...
  describe('통계 계산', () => {
    it('여러 날짜의 통계를 올바르게 계산해야 함', async () => {
      ...
      localStorageMock.setItem('writingHistory', JSON.stringify(multiDateHistory));
    ...
    it('템플릿이 없는 항목도 처리해야 함', async () => {
      ...
      localStorageMock.setItem('writingHistory', JSON.stringify(historyWithoutTemplate));
