...
/**
 * ChatView 컴포넌트
 * PureChatGPTInterface의 채팅 뷰 영역을 분리한 컴포넌트
    ...
    React.useEffect(() => {
        if (messages.length > 0 && !loading) {
            ...
            if (lastMessage?.role === 'assistant' && lastMessage.content) {
                announce(`새 AI 응답이 도착했습니다. ${lastMessage.content.substring(0, 100)}${lastMessage.content.length > 100 ? '...' : ''}`);
            }
        }
        ...
        >
            {currentChat ? (
                ...
                <div className="chat-session-placeholder" aria-live="polite">
                    원하는 프로젝트를 선택하거나 새 채팅을 시작하면 대화가 여기에 표시됩니다.
                </div>
            )}
