...
const WritingStatisticsDashboard: React.FC<WritingStatisticsDashboardProps> = ({ content }) => {
    ...
    const loadStatistics = () => {
        try {
            ...
            history.forEach((item: any) => {
                const date = new Date(item.createdAt || Date.now()).toISOString().split('T')[0];
                const analysis = writingQualityAnalyzer.analyzeQuality(item.content || '');
                ...
                if (item.template) {
                    dayStats.templates.add(item.template);
                }
            });

            dateMap.forEach((dayStats, date) => {
                const historyForDate = history.filter(
                    (item: any) => new Date(item.createdAt || Date.now()).toISOString().split('T')[0] === date
                );
                stats.push({
                    date,
            ...
            const avgQuality = stats.length > 0
                ...
                : 0;
            const templatesUsed = new Set(history.map((item: any) => item.template).filter(Boolean) as string[]);
    ...
    const updateStatistics = (newContent: string) => {
        // 실시간 통계 업데이트는 필요시 구현
        loadStatistics();
    };
                ...
                <div className="daily-stats-section">
                    ...
                    <PredictionChart
                        data={{
                            ...
                            colors: getRecentDays(7).map(() => '#667eea'),
                        }}
                        type="line"
                        title="일별 단어 수"
                    />
                ...
                <div className="quality-trend-section">
                    ...
                    <PredictionChart
                        data={{
                            ...
                            colors: getRecentDays(7).map((s) => (s.qualityScore >= 80 ? '#28a745' : s.qualityScore >= 60 ? '#ffc107' : '#dc3545')),
                        }}
                        type="line"
                        title="일별 품질 점수"
                    />
