...
const AdvancedFeaturesPanel: React.FC<AdvancedFeaturesPanelProps> = ({
  ...
  useEffect(() => {
    const pid = voiceGenProjectId.trim();
  ...
  const handleVoiceGenGenerate = useCallback(async () => {
    const script = voiceGenScript.trim();
    if (!script) {
    ...
    try {
      ...
      const promptPart = voiceGenEmotionPrompt.trim();
      const instructions = [emotionPart, promptPart].filter(Boolean).join(' ').trim() || undefined;
      ...
      if (useSegmentSpeed) {
        ...
        for (let i = 0; i < voiceGenSegments.length; i++) {
          ...
          } else if (voiceGenMode === 'project' && voiceGenProjectId.trim()) {
            blob = await speakQwenTtsFromProject(text, voiceGenProjectId.trim(), {
        ...
        if (voiceGenMode === 'situation') {
          ...
          blob = await speakQwenTtsFromProject(script, voiceGenProjectId.trim(), {
            voiceSourceId: voiceGenSelectedSourceId ?? undefined,
            situation: voiceGenSituation,
            maxRefSeconds: 10,
            speed,
  ...
  const handleVoiceGenKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      const canGenerate =
        ...
        voiceGenScript.trim().length > 0 &&
        (voiceGenMode !== 'url' || voiceGenUrl.trim().length > 0) &&
        (voiceGenMode !== 'project' || voiceGenProjectId.trim().length > 0);
      if (canGenerate) {
        e.preventDefault();
        if (e.shiftKey) {
  ...
  const handleAddProjectVoiceSource = useCallback(async () => {
    const pid = voiceGenProjectId.trim();
    const url = voiceGenAddSourceUrl.trim();
    if (!pid || !url) {
    ...
    try {
      ...
      setError(null);
      const startSec = voiceGenAddSourceStartSec.trim() ? parseFloat(voiceGenAddSourceStartSec) : undefined;
      const endSec = voiceGenAddSourceEndSec.trim() ? parseFloat(voiceGenAddSourceEndSec) : undefined;
      await addProjectVoiceSource(pid, url, {
        refText: voiceGenAddSourceRefText.trim() || undefined,
        name: voiceGenAddSourceName.trim() || undefined,
        referenceUrl: voiceGenAddSourceReferenceUrl.trim() || undefined,
    ...
    async (targetLineIndex?: number) => {
      const indices =
        targetLineIndex != null
          ? [targetLineIndex]
          : voiceGenLines.map((l, i) => i).filter((i) => !voiceGenLines[i].isPause && voiceGenLines[i].text.trim());
      if (indices.length === 0) {
        setError(targetLineIndex != null ? '해당 줄에 텍스트가 없습니다.' : '생성할 줄이 없습니다.');
        return;
      }
      const isSingle = targetLineIndex != null;
      if (voiceGenMode === 'url' && !voiceGenUrl.trim()) {
        setError('영상 URL을 입력해 주세요.');
        return;
      }
      if (voiceGenMode === 'project' && !voiceGenProjectId.trim()) {
      ...
      try {
        ...
        for (const idx of indices) {
          ...
          const linePrompt = line.tonePrompt.trim();
          const emotionPart = voiceGenEmotionMode === 'preset' ? `#${voiceGenEmotionPreset}` : '';
          const globalPrompt = voiceGenEmotionPrompt.trim();
          const instructions = [emotionPart, linePrompt, globalPrompt].filter(Boolean).join(' ').trim() || undefined;
          let blob: Blob;
          if (voiceGenMode === 'situation') {
            blob = await speakQwenTts(text, {
              situation: voiceGenSituationOnly,
            ...
            blob = await speakQwenTtsScriptFromSourceUrl(text, voiceGenUrl.trim(), {
              situation: voiceGenSituation,
              maxRefSeconds: 10,
              speed,
              instructions,
    ...
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (result.success && result.text) {
          ...
          const scriptForVoice = (result.dialogue_only ?? result.text).trim() || result.text;
  ...
  const handleAnalyzeScriptStyle = useCallback(async () => {
    const sample = voiceGenSampleScript.trim();
    if (!sample) {
  ...
  const handleGenerateScriptInStyle = useCallback(async () => {
    const sample = voiceGenSampleScript.trim();
    const topic = voiceGenTopicOutline.trim();
    if (!sample) {
      setError('샘플 대본을 입력하거나 문서에서 추출해 주세요.');
      return;
    }
  ...
  return (
    <div className="advanced-features-panel" role="region" aria-label="고급 기능" data-testid="advanced-features-panel">
      <div className="panel-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 id="advanced-features-heading">고급 기능</h2>
          <div className="connection-status" role="status" aria-live="polite" aria-label={wsConnected ? '실시간 연결됨' : '연결 끊김'}>
            <span className={`status-indicator ${wsConnected ? 'connected' : 'disconnected'}`}></span>
            <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-secondary)', marginLeft: 'var(--spacing-sm)' }}>
      ...
      >
        ...
        <button
          ...
          className={`tab-button ${activeTab === 'voiceGen' ? 'active' : ''}`}
      ...
      <div className="panel-content">
        ...
        {/* 음성 인식 탭 */}
        {activeTab === 'voice' && (
          <div id={PANEL_IDS.voice} className="voice-recognition-section" role="tabpanel" aria-labelledby={TAB_IDS.voice}>
            <h3>음성 인식</h3>
        ...
        {/* 이미지 분석 탭 */}
        {activeTab === 'image' && (
          <div id={PANEL_IDS.image} className="image-analysis-section" role="tabpanel" aria-labelledby={TAB_IDS.image}>
            <h3>이미지 분석</h3>
        ...
        {activeTab === 'prediction' && (
          <div id={PANEL_IDS.prediction} className="prediction-analysis-section" role="tabpanel" aria-labelledby={TAB_IDS.prediction}>
        ...
        {/* 목소리 생성 탭 (Qwen TTS) */}
        {activeTab === 'voiceGen' && (
          <div id={PANEL_IDS.voiceGen} className="voice-gen-section" data-testid="voice-gen-section" role="tabpanel" aria-labelledby={TAB_IDS.voiceGen}>
              ...
              <>
                ...
                {voiceGenMode === 'url' && (
                  <div className="control-group">
                    ...
                    <p className="voice-gen-helper" role="status" style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '6px' }}>
                    ...
                    <input
                      ...
                      placeholder="https://www.youtube.com/... 또는 https://www.tiktok.com/..."
                ...
                {voiceGenMode === 'situation' && (
                  <div className="control-group">
                    ...
                    <select
                      ...
                      className="message-input"
                      style={{ display: 'block', marginTop: '4px', maxWidth: '320px' }}
                    >
                ...
                {voiceGenMode !== 'project' && (
                  <details className="control-group" style={{ marginBottom: '8px' }} data-testid="voice-gen-advanced-project">
                    <summary style={{ cursor: 'pointer', fontSize: 'var(--font-size-sm)', color: 'var(--text-secondary)' }}>고급: 프로젝트 보이스 소스</summary>
                ...
                {voiceGenProjectId.trim() && voiceGenMode === 'project' && (
                  <div className="control-group voice-gen-project-sources">
                    ...
                    <p className="voice-gen-helper" role="status" style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '12px' }}>
                    ...
                    <div className="voice-gen-add-source">
                      <div className="control-group" style={{ marginBottom: '8px' }}>
                        ...
                        <p className="voice-gen-helper" role="status" style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px' }}>
                      ...
                      <div className="control-group" style={{ marginBottom: '8px' }}>
                        <span style={{ fontWeight: 600, marginBottom: '4px', display: 'block' }}>해당 목소리만 구간 (여러 사람 나올 때)</span>
                        <p className="voice-gen-helper" role="status" style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '6px' }}>
                      ...
                      <button
                        ...
                        disabled={loadingState.type === 'updating' || !voiceGenAddSourceUrl.trim()}
                ...
                <details className="control-group" style={{ marginTop: '12px' }}>
                  <summary style={{ cursor: 'pointer', fontSize: 'var(--font-size-sm)', color: 'var(--text-secondary)' }}>같은 스타일로 새 대본 만들기 (선택)</summary>
                <div className="control-group" role="group" aria-label="샘플 대본으로 스타일 반영">
                  <span style={{ fontWeight: 600, marginBottom: '8px', display: 'block' }}>
                    샘플 대본으로 톤·스타일·어투 반영
                  </span>
                  <p className="voice-gen-helper" role="status" style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                    ...
                    <label htmlFor="voice-gen-sample-script" style={{ fontSize: '12px', marginLeft: '8px' }}>또는 샘플 대본 붙여넣기:</label>
                  </div>
                  <textarea
                  ...
                  />
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                    <label htmlFor="voice-gen-document-hint" style={{ fontSize: '12px', minWidth: '90px' }}>문서 유형 힌트</label>
                    <select
                    ...
                    {voiceGenSourceFilename && (
                      <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }} title={voiceGenSourceFilename}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                    <button
                      ...
                      disabled={loadingState.type === 'updating' || !voiceGenSampleScript.trim()}
                      data-testid="voice-gen-analyze-style"
                    >
                      {loadingState.type === 'updating' ? '분석 중...' : '스타일 분석'}
                    </button>
                    {voiceGenStyleSummary && (
                      <details style={{ marginLeft: '8px', flex: '1 1 200px' }}>
                        <summary style={{ cursor: 'pointer', fontSize: '12px' }}>분석 결과 보기</summary>
                        <div style={{ marginTop: '6px', padding: 'var(--spacing-sm)', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', fontSize: 'var(--font-size-xs)', whiteSpace: 'pre-wrap', maxHeight: '120px', overflowY: 'auto' }}>
                          {voiceGenStyleSummary}
                        </div>
                      </details>
                    )}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    <label htmlFor="voice-gen-topic-outline" style={{ fontSize: '12px', minWidth: '120px' }}>생성할 주제/개요</label>
                    <input
                    ...
                    <button
                      ...
                      disabled={loadingState.type === 'updating' || !voiceGenSampleScript.trim() || !voiceGenTopicOutline.trim()}
                      ...
                      <div className="control-group voice-gen-script-block">
                        ...
                        {!voiceGenLineMode && (
                          <button
                            ...
                            title={voiceGenScript.trim() ? '대본을 줄 단위로 나눕니다' : '대본을 입력한 뒤 변환해 주세요'}
                          >
                                      ...
                                      {!line.isPause && (
                                        <>
                                          <button
                                            ...
                                            disabled={loadingState.type === 'updating' || !line.text.trim()}
                    ...
                    <section className="voice-gen-step voice-gen-step--options" aria-label="옵션 및 생성">
                      {voiceGenLineMode && voiceGenSelectedLineIndex != null && (
                        <div className="voice-gen-line-settings voice-gen-typecast-sections" data-testid="voice-gen-line-settings">
                          <div className="voice-gen-typecast-select-input">
                            ...
                            {voiceGenInputPreset !== '일반' && (
                              <>
                                ...
                                <div className="voice-gen-preset-actions" style={{ display: 'flex', gap: '8px', marginTop: '8px', flexWrap: 'wrap' }}>
                                  <button
                                    type="button"
                                    className="btn btn-secondary btn-sm"
                          ...
                          {voiceGenLines[voiceGenSelectedLineIndex]?.isPause ? (
                            <div className="voice-gen-voice-group">
                              ...
                              <div className="voice-gen-pause-stepper" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <button
                                  type="button"
                          ...
                          <div className="voice-gen-voice-group">
                            ...
                            >
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px', flexWrap: 'wrap' }}>
                                ...
                                />
                                <span style={{ fontSize: '12px' }}>초</span>
                                <button
                                  type="button"
                                  className="btn btn-secondary btn-sm"
                                  onClick={handleApplyReadingTime}
                                  disabled={voiceGenLines[voiceGenSelectedLineIndex]?.isPause || !voiceGenReadingTimeInput.trim()}
                          ...
                          <div className="voice-gen-voice-group voice-gen-typecast-group">
                            ...
                            />
                            <div className="voice-gen-tone-tags" style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                              {TONE_HASHTAGS.map((tag) => (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    const next = cur.includes(tag) ? cur.replace(tag, '').trim() : `${cur} ${tag}`.trim();
                          ...
                          <div className="voice-gen-voice-group voice-gen-typecast-group">
                            ...
                            <div className="voice-gen-typecast-pause-row">
                              ...
                              />
                              <span style={{ fontSize: '11px', color: 'var(--text-tertiary)' }}>0초 — 10.0초</span>
                            </div>
                            <button
                              type="button"
                              className="btn btn-secondary btn-sm"
                          ...
                          <div className="voice-gen-voice-group voice-gen-typecast-group">
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              />
                              <span style={{ fontSize: '12px', minWidth: '2.5em' }}>{voiceGenPlaybackRate.toFixed(1)}배</span>
                            </div>
                          </div>
                          <div className="voice-gen-voice-group voice-gen-typecast-group">
                          ...
                          <div className="voice-gen-voice-group voice-gen-typecast-group" role="group" aria-label="감정 제어 Typecast 스타일">
                            ...
                            <p className="voice-gen-helper" style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                          ...
                          <div className="voice-gen-voice-group voice-gen-typecast-group" role="group" aria-label="감정·상황 프롬프트">
                            ...
                            <p className="voice-gen-helper" style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '6px' }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                              {VOICE_GEN_EMOTION_TAGS.map((tag) => (
                                <button
                                  ...
                                  className="btn btn-secondary btn-sm"
                                  onClick={() => {
                                    const cur = voiceGenEmotionPrompt.trim();
                                    const next = cur.includes(tag) ? cur.replace(tag, '').trim().replace(/\s+/g, ' ') : `${cur} ${tag}`.trim();
                        ...
                        <div className="control-group" role="group" aria-label="재생 속도 (전체)">
                          ...
                          />
                          <span style={{ marginLeft: '8px', fontSize: '12px' }}>{voiceGenGlobalSpeed.toFixed(2)}x</span>
                        </div>
                      </div>
                      {voiceGenLineMode && (
                        <div className="voice-gen-voice-group voice-gen-typecast-group">
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <input
                              ...
                              style={{ flex: 1 }}
                            />
                            <span style={{ fontSize: '12px' }}>{voiceGenPlaybackRate.toFixed(1)}배</span>
                          </div>
                        </div>
                      )}
                      {voiceGenMode !== 'situation' && (
                      ...
                      <div className="voice-gen-voice-group">
                        ...
                        {voiceGenUseSegmentSpeed && (
                          <>
                            <div className="control-group" style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginTop: '8px' }}>
                            ...
                            {voiceGenSegments.length > 0 && (
                              <div className="control-group" style={{ maxHeight: '160px', overflowY: 'auto', marginTop: '8px' }}>
                                <span style={{ fontWeight: 600, marginBottom: '6px', display: 'block', fontSize: 'var(--font-size-xs)' }}>구간별 속도 ({voiceGenSegments.length}개)</span>
                                {voiceGenSegments.map((seg, i) => (
                                  <div key={i} style={{ marginBottom: '6px', padding: '6px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-sm)' }}>
                                    <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={seg.text}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                      ...
                                      />
                                      <span style={{ fontSize: '11px', minWidth: '32px' }}>{seg.speed.toFixed(2)}x</span>
                                    </div>
                                  </div>
                                ))}
                              </div>
                      ...
                      <div className="voice-controls" style={{ marginTop: '12px' }}>
                        {(() => {
                          const isDisabled =
                            loadingState.type === 'updating' ||
                            !voiceGenScript.trim() ||
                            (voiceGenMode === 'url' && !voiceGenUrl.trim()) ||
                            (voiceGenMode === 'project' && !voiceGenProjectId.trim());
                          ...
                          else if (!voiceGenScript.trim()) buttonTitle = '대본을 입력해 주세요';
                          else if (voiceGenMode === 'url' && !voiceGenUrl.trim()) buttonTitle = '영상 URL을 입력해 주세요';
                          else if (voiceGenMode === 'project' && !voiceGenProjectId.trim()) buttonTitle = '프로젝트 ID를 입력해 주세요';
                ...
                {(voiceGenAudioUrl || voiceGenSegmentUrls.length > 0) && (
                  <section className="voice-gen-result" aria-label="생성 결과">
                    <h4 className="voice-gen-result-title">생성된 음성</h4>
