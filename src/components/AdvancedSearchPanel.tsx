...
const AdvancedSearchPanel: React.FC<AdvancedSearchPanelProps> = ({
  ...
  useEffect(() => {
    if (debouncedSearchTerm.trim().length > 0) {
      ...
      // 검색 히스토리에 저장
      searchHistoryService.saveSearch(debouncedSearchTerm, results.length);
    } else {
      setResults([]);
      onSearchTermChange?.('');
    }
  }, [debouncedSearchTerm, performSearch, onSearchTermChange, results.length]);
  ...
  const handleSaveSearch = useCallback(() => {
    if (!searchTerm.trim() || !saveSearchName.trim()) return;
  ...
  return (
    ...
    >
      <div className="advanced-search-panel" onClick={(e) => e.stopPropagation()} role="document">
        ...
        {/* 검색 히스토리 (검색어가 없을 때) */}
        {showHistory && searchTerm.trim().length === 0 && (
          <div className="search-history-panel">
            {recentSearches.length > 0 && (
              <div className="history-section">
                <div className="history-header">
                  <span className="history-title">최근 검색어</span>
                  <button className="history-clear-btn" onClick={handleClearHistory} title="전체 삭제">
                    전체 삭제
                  </button>
                </div>
              ...
              {result.metadata && (
                <div className="result-metadata">
                  ...
                  {result.metadata.sessionId && (
                    <span className="result-tag">세션: {result.metadata.sessionId.substring(0, 8)}</span>
                  )}
                </div>
              )}
            </div>
              ...
              {searchStats.mostSearchedTerms.length > 0 && (
                <div className="analytics-section">
                  ...
                  <div className="popular-terms-list">
                    {searchStats.mostSearchedTerms.slice(0, 5).map((term: any, index: number) => (
              ...
              {Object.keys(searchStats.timeDistribution).length > 0 && (
                <div className="analytics-section">
                  ...
                  <div className="time-distribution">
                    {Object.entries(searchStats.timeDistribution).map(([timeSlot, count]: [string, any]) => (
