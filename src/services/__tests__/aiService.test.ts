...
describe('AIService', () => {
  ...
  describe('고급 NLP 분석', () => {
    ...
    it('NLP 분석 실패 처리', async () => {
      // generateSummary와 generateRecommendations에서 사용하는 fetch를 모킹
      (global.fetch as jest.Mock).mockRejectedValue(new Error('Error'));

      // analyzeWithNLP는 내부적으로 generateSummary와 generateRecommendations를 호출하므로
      // 이들이 에러를 throw하면 analyzeWithNLP도 에러를 throw합니다
      try {
        await service.analyzeWithNLP('테스트', 'gemini-pro');
        // 에러가 발생해야 하는데 발생하지 않았으면 테스트 실패
