...
import { newsService, NewsArticle, CommentAnalysis } from '../newsService';
...
describe('newsService', () => {
  ...
  describe('setAPIKey', () => {
    it('API 키를 설정할 수 있어야 함', () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith('news_api_key', 'new-api-key');
    });
  });
  ...
  describe('searchNews', () => {
    ...
    it('API 키가 없으면 에러를 던져야 함', async () => {
      // API 키 제거
      const serviceWithoutKey = new (newsService.constructor as any)();
