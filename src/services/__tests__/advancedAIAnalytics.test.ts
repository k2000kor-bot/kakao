  ...
  describe('runAdvancedAnalysis', () => {
    ...
    it('세션 컨텍스트를 활용할 수 있어야 함', async () => {
      const mockSession: ChatSession = {
        ...
        title: '테스트 세션',
        messages: [
          { id: '1', content: '첫 번째 메시지', isUser: true, timestamp: new Date() },
          { id: '2', content: '두 번째 메시지', isUser: false, timestamp: new Date() },
        ],
      };
    ...
    it('프로젝트 컨텍스트를 활용할 수 있어야 함', async () => {
      ...
      expect(analysis.context).toBeDefined();
      expect(analysis.context.project).toBe('개포우성7차 재개발');
    });
