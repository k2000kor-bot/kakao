...
import advancedAIGovernanceEthicalSystem, {
  AIGovernancePolicy,
  EthicalAIAnalysis,
  GovernanceViolation,
  GovernanceMetrics,
  AuditEntry,
} from '../advancedAIGovernanceEthicalSystem';
  ...
  describe('resolveViolation', () => {
    it('위반을 해결할 수 있어야 함', async () => {
      ...
      if (analysis.violations.length > 0) {
        ...
        // 위반이 해결되었거나, 같은 rule_id를 가진 위반이 없어야 함
        // resolveViolation은 rule_id로 첫 번째 매칭되는 위반을 해결하므로
        // 같은 rule_id를 가진 위반이 여러 개일 수 있음
