...
describe('PersistentChatSessionService', () => {
  ...
  beforeEach(() => {
    ...
    (service as any).sessionCounter = 0;
  });
  ...
  describe('세션 변환', () => {
    it('일반 세션을 지속적 세션으로 변환', () => {
      ...
      // 세션을 먼저 추가
      (service as any).sessions.set('session-1', mockSession);
  ...
  describe('세션 검색', () => {
    it('제목으로 세션 검색', () => {
      ...
      (service as any).sessions.set('session-1', mockSession1);
      (service as any).sessions.set('session-2', mockSession2);
