  ...
  describe('executeIntelligentWorkflow', () => {
    ...
    it('기본 워크플로우를 실행할 수 있어야 함', async () => {
      ...
      try {
        ...
        expect((response as any).workflowId).toBeDefined();
      } catch (error) {
    ...
    it('캐싱된 결과를 반환할 수 있어야 함', async () => {
      ...
      try {
        ...
        expect((response2 as any).workflowId).toBeDefined();
      } catch (error) {
        expect(error).toBeDefined();
      }
    });
  ...
  describe('실제 사용자 질문/요구 시나리오 테스트', () => {
    ...
    it('긴급한 질문에 대해 빠르게 응답할 수 있어야 함', async () => {
      ...
      try {
        ...
        expect((response as any).metadata.executionTime).toBeGreaterThanOrEqual(0);
      } catch (error) {
