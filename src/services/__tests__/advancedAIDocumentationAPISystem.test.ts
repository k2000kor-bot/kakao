...
import advancedAIDocumentationAPISystem, {
  APIDocumentation,
  APIEndpoint,
  DocumentationMetrics,
  APIUsageStats,
} from '../advancedAIDocumentationAPISystem';
...
describe('advancedAIDocumentationAPISystem', () => {
  ...
  describe('searchEndpoints', () => {
    ...
    it('경로로 엔드포인트를 검색할 수 있어야 함', async () => {
      await advancedAIDocumentationAPISystem.generateAPIDocumentation('integrated-ai-service');
      const results = advancedAIDocumentationAPISystem.searchEndpoints('/api/ai');
  ...
  describe('문서화 품질 검사', () => {
    it('문서화 품질이 낮을 때 알림을 생성해야 함', async () => {
      const documentation = await advancedAIDocumentationAPISystem.generateAPIDocumentation('quality-test');
