...
class MockVideo {
  ...
  src: string = '';
  onloadedmetadata: ((event: any) => void) | null = null;
  onerror: ((event: any) => void) | null = null;
...
(global as any).HTMLVideoElement = MockVideo;
...
describe('MediaAnalysisService', () => {
  ...
  describe('파일 ID 생성', () => {
    it('파일 ID 생성 확인', async () => {
      ...
      expect(result.id).toBeDefined();
      expect(result.id).toContain('test-file');
    });
  ...
  describe('에러 처리', () => {
    it('에러 발생 시 처리', async () => {
      ...
      (global as any).Image = MockImageError;
