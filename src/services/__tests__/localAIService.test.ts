...
describe('LocalAIService', () => {
  ...
  describe('프로젝트 메시지 처리', () => {
    it('프로젝트 메시지 처리 - 특정 프로젝트', async () => {
      const request: LocalAIRequest = {
        type: 'project',
        content: '개포우성7차 프로젝트 정보',
      };

      const response = await service.processMessage(request);

      expect(response.success).toBe(true);
      expect(response.message.content).toContain('개포우성7차');
      expect(response.metadata?.model).toBe('local-ai');
    });
    ...
    it('잠실우성 프로젝트 조회', async () => {
      ...
      expect(response.success).toBe(true);
      expect(response.message.content).toContain('잠실우성');
    });
  });
  ...
  describe('에러 처리', () => {
    it('에러 발생 시 폴백 응답', async () => {
      ...
      const originalHandleChatMessage = (service as any).handleChatMessage;
      (service as any).handleChatMessage = jest.fn().mockRejectedValue(new Error('Test error'));
      ...
      // 원래 메서드 복원
      (service as any).handleChatMessage = originalHandleChatMessage;
    });
  });
  ...
  describe('타입별 처리', () => {
    ...
    it('알 수 없는 타입은 기본 채팅으로 처리', async () => {
      const request = {
        type: 'unknown' as any,
        content: '테스트',
      };
