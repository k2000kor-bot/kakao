...
describe('fileUploadService', () => {
  ...
  describe('uploadFile', () => {
    it('파일을 업로드할 수 있어야 함', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/sessions/session-123/upload'),
  ...
  describe('getFileAnalysis', () => {
    it('파일 분석 결과를 조회할 수 있어야 함', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/sessions/session-123/files/file-123/analysis')
      );
    });
  ...
  describe('getUploadedFiles', () => {
    it('업로드된 파일 목록을 조회할 수 있어야 함', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/sessions/session-123/files')
      );
    });
  ...
  describe('deleteFile', () => {
    it('파일을 삭제할 수 있어야 함', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/sessions/session-123/files/file-123'),
        expect.objectContaining({
          method: 'DELETE',
        })
      );
