...
class SecurityAutomationService {
    ...
    private async checkRules(): Promise<void> {
        ...
        try {
            ...
            errorLogger.error('자동화 규칙 확인 실패', error as Error, {
        ...
        switch (trigger.type) {
            case 'event':
                ...
                if (trigger.condition.includes('risk_level == high')) {
                    return context.events.some((e) => e.risk_level === 'high');
                }
                break;
            ...
            case 'threat':
                if (trigger.severity) {
                    return context.threats.some(
                        (t) => t.severity === trigger.severity && t.status !== 'resolved'
                    );
                }
                break;
            ...
            case 'metric':
                ...
                if (trigger.condition.includes('security_score <')) {
                    ...
                    return context.status.security_score < threshold;
                }
                break;
        }
    ...
    private async executeRule(rule: AutomationRule): Promise<void> {
        ...
        try {
            ...
            console.log(`규칙 실행 완료: ${rule.name} (총 ${rule.trigger_count}회)`);
        } catch (error) {
            errorLogger.error(`규칙 실행 실패: ${rule.name}`, error as Error, {
                component: 'SecurityAutomationService',
                action: 'executeRule',
                rule_id: rule.id,
            });
            ...
            case 'block_ip':
                ...
                if (failedLogins.length > 0) {
                    ...
                    if (topIP && topIP[1] >= 5) {
                        ...
                        console.log(`IP 자동 차단: ${topIP[0]}`);
                    }
                }
                break;

            case 'run_scan':
                await advancedSecurityService.runSecurityScan(action.params.scan_type || 'full');
                console.log(`보안 스캔 실행: ${action.params.scan_type}`);
                break;

            case 'send_alert':
                // 알림은 WebSocket을 통해 전송되거나 로깅
                console.log(`보안 알림: ${action.params.severity || 'medium'}`);
                break;

            case 'notify_admin':
                console.log(`관리자 알림: ${action.params.priority || 'normal'}`);
                break;

            case 'update_policy':
                // 정책 업데이트는 수동으로 처리
                console.log(`정책 업데이트 요청: ${JSON.stringify(action.params)}`);
                break;
        }
    }
