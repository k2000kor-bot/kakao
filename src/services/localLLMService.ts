...
export class LocalLLMService {
  ...
  private saveProviders(): void {
    try {
      ...
      localStorage.setItem('localLLMProviders', JSON.stringify(providersArray));
    } catch (error) {
  ...
  private loadProviders(): void {
    try {
      const saved = localStorage.getItem('localLLMProviders');
      if (saved) {
  ...
  private saveProjectConfigs(): void {
    try {
      ...
      localStorage.setItem('projectLLMConfigs', JSON.stringify(configsArray));
    } catch (error) {
  ...
  private loadProjectConfigs(): void {
    try {
      const saved = localStorage.getItem('projectLLMConfigs');
      if (saved) {
