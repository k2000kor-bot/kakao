...
| **배포 전 스크립트(`verify:final`)** | `npm run verify:final` → `scripts/final-verify.sh` (`check:test-imports` 선행·빌드·접속·API·통합 시도 후 **`test:frontend:chat-pipeline`** 필수 — [docs/FINAL_CHECKLIST.md](docs/FINAL_CHECKLIST.md). UI 스모크 npm 스크립트는 없음; 채팅 UI는 `test:sidebar-context` 등 별도; 수동 §14.5 [CHAT_UI_TEST_SCENARIOS](docs/guides/CHAT_UI_TEST_SCENARIOS.md)) |
| 전체 점검 | `npm run dev:check` |
| 프론트만 (타입+린트) | `npm run dev:check:frontend` |
| 타입만 (빠른 검사) | `npm run quick-check` (tsconfig.build.json, 테스트 제외) |
| P4 서비스 (9 suites, 158 tests) | `npm run test:p4:services` — [docs/COMPLETION_CHECKLIST.md](docs/COMPLETION_CHECKLIST.md) §3.2 · 대화 관계도 클라이언트: [docs/CONVERSATION_GRAPH.md](docs/CONVERSATION_GRAPH.md) |
| **대화 관계도 (통합)** | `npm run verify:conversation-graph:unit` — `typecheck:views-tests` · Jest 169+ · 백엔드 6 tests · `make test-conversation-graph-unit`. E2E 포함: `verify:conversation-graph`(12 chromium). CI: `ci-cd.yml` `conversation-graph`. `/chat` handoff: `test:conversation-graph:chat-handoff`. API: `verify:conversation-graph-api` |
| **소스 탭 파일 업로드 (통합)** | `npm run verify:project-sources` — `projectService`·`ChatGPTInterface` 유닛 · 백엔드 `POST /api/projects/{id}/files` · E2E `e2e/projectManagement.spec.ts` 「소스 탭에서 파일 업로드」(`E2E_SERVER_READY=1`) |
| 확장 뷰·라우트 (뷰 유닛 + routes.test) | `npm run test:views` (22 suites, 137 tests) |
| **라우트 설정만 (`routes.test`)** | `npm run test:routes` — `src/config/__tests__/routes.test.ts` **27** tests (`pretest` 포함) |
| **통합 앱 셸만 (`AppUnified.test`)** | `npm run test:app-unified` — `src/AppUnified.test.tsx` **122** tests (`pretest` 포함, 수 초~15초대) |
| **사이드바 컨텍스트·설정·대화 이력 회귀** | `npm run test:sidebar-context` — 위 네 스위트 묶음(`pretest`·`sync:frontend-src` 포함; 동일 `make test-sidebar-context`) — [docs/PUSH_BLOCK_HANDOFF.md](docs/PUSH_BLOCK_HANDOFF.md). Jest 외 **수동**(첨부·재생성·편집): [docs/guides/CHAT_UI_TEST_SCENARIOS.md](docs/guides/CHAT_UI_TEST_SCENARIOS.md) §14.5·§14.10 · [docs/guides/CHAT_ANSWER_FLOW_VERIFICATION.md](docs/guides/CHAT_ANSWER_FLOW_VERIFICATION.md) §8 행 9 |
...
| **ChatGPTInterface 등** | `npm run test:sidebar-context`(묶음 회귀 권장) 또는 `npm test -- --testPathPattern='ChatGPTInterface\\.test\\.tsx$' --watchAll=false`(파일 하나만; `$`로 Ultimate 파일 오매칭 방지) — 수동 §14.5 [CHAT_UI_TEST_SCENARIOS](docs/guides/CHAT_UI_TEST_SCENARIOS.md) |
| **컴포저(다중 요청·첨부·순차·다단계)** | `npm run test:composer-pipeline` — 전송·**재생성·편집** 공유(`composerSequentialSend`). **삭제·초기화 후 첨부+짧은 지시**: `composerContextAfterClear.test`(Jest). 순차: `REACT_APP_COMPOSER_SEQUENTIAL_MULTI_REQUEST`(+ `..._STREAM` SSE). 다단계: `REACT_APP_COMPOSER_MULTI_STEP_MULTI_REQUEST`(순차보다 우선순위 낮음). Jest: `npm run verify:composer-pipeline`(20 suites — Council·자가 개발·`composerRegenerateTurn`·`composerContextAfterClear` 포함) · E2E 로컬(서버 선기동): `test:e2e:composer-pipeline:all`(에이전트·다중요청·재생성) · **첨부 맥락 E2E**: `test:e2e:composer-attach-context` · E2E CI: `test:e2e:composer-pipeline:ci:all` · 재생성만: `test:e2e:composer-regenerate` · CI: `composer-pipeline` · `composer-pipeline-e2e` · `composer-regenerate-e2e` |
...
**ChatGPTInterface Jest 경로**: `--testPathPattern`에 `ChatGPTInterface.test`처럼 **파일명 부분 문자열**만 주면 `UltimateChatGPTInterface.test.tsx`까지 매칭될 수 있음 → **`ChatGPTInterface\\.test\\.tsx$`** 로 끝을 고정하거나 **`npm run test:sidebar-context`** 묶음을 권장(수동 §14.5 [CHAT_UI_TEST_SCENARIOS](docs/guides/CHAT_UI_TEST_SCENARIOS.md)).
...
**대화 API**: `/api/chat`, `/api/chat/stream`. 전달 필드: `context.conversation_history`(최근 20턴), `project_id`, `request_id`, `diversity`, `temperature`. 프로젝트 맥락: `context.project_files`(참고 파일 목록), `context.project_instructions`(프로젝트 지침) — 백엔드에서 projectKnowledge에 반영. 일회·스레드 첨부 본문: `context.conversation_file_content`·`conversation_file_name`·`thread_attached_file_contents`(메인 전송·재생성·편집 동일) — [docs/guides/CHAT_CONTEXT_CONTRACT.md](docs/guides/CHAT_CONTEXT_CONTRACT.md) §1·§2.
...
**수동(대화 첨부·재생성·편집)**: [docs/guides/CHAT_UI_TEST_SCENARIOS.md](docs/guides/CHAT_UI_TEST_SCENARIOS.md) §14(**§14.5** — 스레드 또는 입력창 .txt/.csv 후 짧은 질문만 보내고 **재생성** 또는 메시지 **편집 후 재전송**·`context`·응답 맥락). 전체 흐름·표 **행 9**: [docs/guides/CHAT_ANSWER_FLOW_VERIFICATION.md](docs/guides/CHAT_ANSWER_FLOW_VERIFICATION.md) §5.6·§8. 기능 요약: [docs/FEATURE_LOGIC_AND_STRENGTHS.md](docs/FEATURE_LOGIC_AND_STRENGTHS.md) §3.5·§5.3.
**빠른 검증**: 라우트·통합 셸만 먼저 — `npm run test:routes`(**27**), `npm run test:app-unified`(**122**, 수 초대). 사이드바·설정·대화·ChatGPT 회귀는 `npm run test:sidebar-context`(수동 §14.5 [CHAT_UI_TEST_SCENARIOS](docs/guides/CHAT_UI_TEST_SCENARIOS.md)). ① `npm run test:p4:services` → 9 suites, 158 tests. ② `npm test -- --testPathPattern="config/__tests__|store/__tests__/projectsSlice|store/__tests__/uiSlice|formatters.test|ProjectHub.test" --watchAll=false` → 6 suites, 110 tests. **수동(첨부·재생성·편집)** 점검은 앞 문단 CHAT_UI §14.5·CHAT_ANSWER §8 행 9 참고.
...
| **서비스 통합** | API·대화·프로젝트·파일 학습·대화 업로드/관계도 | `npm run test:p4:services` (9 suites, 158 tests) | projectService·chatService·fileLearningService·notebookLLMStreamingService·pipelineTuningService·**conversationGraphService** 등 |
...
| `npm run test:p4:services` | notebookLLMStreamingService·chatService·fileLearningService·api·intentApi·projectService·pipelineTuningService·**conversationGraphService** | 대화·프로젝트·파일·관계도 API 클라이언트 |
