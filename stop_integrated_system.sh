...
if [ -f ".integrated_backend.pid" ]; then
    BACKEND_PID=$(cat .integrated_backend.pid)
    echo "🔧 통합 백엔드 서버를 중지합니다 (PID: $BACKEND_PID)..."
...
if [ -f ".legacy_backend.pid" ]; then
    ...
    echo "🔧 기존 백엔드 서버를 중지합니다 (PID: $LEGACY_PID)..."
...
echo "   - 통합 API 서버 (포트 5002)"
echo "   - 기존 API 서버 (포트 5000)"
echo ""
echo "🔄 시스템을 다시 시작하려면: ./start_integrated_system.sh"
echo ""
