...
- **마무리 검증**: `npm run verify:completion` 통과 시 완성도 검증 완료 (타입·린트·P4 170 tests).
...
| AppUnified Jest (`test:app-unified`) | `npm run test:app-unified` — **115** tests(본문 건너뛰기·404·brainwave·/projects·/projects/:id·/voice-generation·확장 뷰·구 URL 리다이렉트 등). 과거 요약 6 시나리오는 스위트 확장으로 통합됨 · 표 행과 교차 |
...
| 대화 질문 답변·대화 맥락 | ChatGPTInterface 응답 추출·백엔드 빈 응답 방지. context.conversation_history(최근 20턴)·대화방 재진입 시 conversations 우선·재생성·편집 경로 동일·consistency_instruction. available_capabilities·buildFeatureContextFromMessage. 백엔드 _run_pre_generation_pipeline. CHAT_ANSWER_FLOW_VERIFICATION §5.6. · 표 행과 교차 |
...
| 입력창 엔터 → 딥러닝 보강 → 딥시크 답변 | sendMessage 단일 진입점. buildMessageToSendForChat(의도·감정·주제 분석·보강) → 백엔드 딥시크. 메인 대화·재생성·편집·노트북(딥시크) 공통. 응답 품질 분석(analyzeResponseWithDL) 비차단. DEEPSEEK_DEVELOPMENT_ORDER §프론트엔드. ChatGPTInterface·NotebookLLM 테스트 mock 반영. · 표 행과 교차 |
...
| 프로젝트 파일 업로드 | POST /api/projects/{id}/files 백엔드·프론트 연동. projectService.uploadProjectFile, ProjectEditModal 파일 추가(API·로컬 폴백·토스트)·업로드 중 로딩. project-edit-file-add E2E, ProjectEditModal 22 tests, projectService uploadProjectFile 3 tests. BACKLOG 85~90차. · 표 행과 교차 |
...
| TTS·통합 앱 | ✅ | TTS 252+11 (test:tts:all), AppUnified 5, scriptStyleAPI 100% · 표 행과 교차 |
| P4 추가 테스트 (2026-02-20) | ✅ | `npm run test:p4:services`: **8 suites, 170 tests** (위 7 + **pipelineTuningService**). notebookLLMStreamingService, chatService, chatGPTProjectService, fileLearningService, api, intentApi, projectService. projectService getProjects data null 폴백 검증 포함. · 표 행과 교차 |
...
| 테스트 검증 (2026-02-15) | ✅ | P4 148 + config/store/formatters/ProjectHub 104 통과. TESTING_GUIDE 빠른 검증 패턴 추가. PerformanceOptimizer.test toError 모킹 수정(13 tests). Task B4: projectAnalyticsService·ProjectHub 50 tests, projectShareService·ProjectShareDialog 42 tests. · 표 행과 교차 |
...
**추가 테스트 검증 (2026-02-20 이후)**: P4 서비스 **8 suites, 170 tests** — `npm run test:p4:services` (pipelineTuningService 포함). projectService 포함, getProjects API data null 시 로컬 폴백 테스트 추가. 백엔드: `npm run test:backend` → 144 passed.
...
| **배포 직전 풀 스택** | `npm run verify:final` → `scripts/final-verify.sh` ([FINAL_CHECKLIST.md](./FINAL_CHECKLIST.md)) · 표 행과 교차 |
| **배포 직전(UI 스모크 순차)** | `npm run verify:final:sequential-smoke` · 표 행과 교차 |
...
| **P4 서비스 (8 suites, 170 tests)** | `npm run test:p4:services` · 표 행과 교차 |
...
| 통합 앱 테스트 | `npm run test:app-unified` 권장(`pretest`·**115** tests). 또는 `npm test -- --testPathPattern="AppUnified" --watchAll=false` · 표 행과 교차 |
...
| ChatGPTInterface (Jest) | 전체: `npm run test:chatgpt-interface`. 레이아웃·패널 접힘 등 일부: `npm run test:chatgpt-interface:quick`. 에이전트 라우트 세션만: `npm run test:chatgpt-interface:genspark`. 답변 흐름 일부: `npm run test:chatgpt-interface -- --testNamePattern="답변 로직 연결" --watchAll=false`. `npm test … ChatGPTInterface.test`·`--testPathPattern=ChatGPTInterface.test`만 쓰면 `UltimateChatGPTInterface.test.tsx`까지 부분 일치할 수 있음 — [TESTING_GUIDE.md](../TESTING_GUIDE.md) **ChatGPTInterface Jest 경로** · 표 행과 교차 |
...
| 2 | P4 서비스 | `npm run test:p4:services` | 8 suites, 170 tests · 표 행과 교차 |
