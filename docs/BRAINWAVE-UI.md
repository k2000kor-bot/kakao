...
- **적용 원칙**: Figma **전체 파일**을 기준으로, **전체 디자인 레이아웃**을 위 링크와 동일하게 맞추고 그대로 적용. 메인 대화·간단 대화·사이드바·환영 화면·입력·버튼·모달·분석·고급 기능 등 **모든 기능이 이 Kit과 동일한 디자인 범위**에 들어가야 함. 새 기능 추가 시에도 `.bw-*`·`theme.css` 변수만 사용.
...
- **전체 기능**: 3분할 라우트 (일반 대화·프로젝트·프로젝트 · 대화) + 도구
  - **대화** (`/`) — ChatGPTInterface (일반 대화·TTS·음성·이미지·예측 등 통합)
  - **프로젝트** (`/projects`) — ProjectsPage (목록·생성·관리)
  - **프로젝트 · 대화** (`/projects/:id`) — ChatGPTInterface (노트북 LLM·파일 분석·AI 문서, 프로젝트 클릭 시 진입)
  - **목소리 생성** (`/voice-generation`) — VoiceGenerationView (TTS 전용). 질문·요구는 대화에 입력 시 해당 기능으로 답변
...
| **통합 앱** | `src/AppUnified.tsx` | 3단 레이아웃(다크 좌측 사이드바·메인·우측 대화 히스토리), CORBU.AI 네비(Chats·Search·Manage subscription·Updates & FAQ·Settings), Chat list, theme.css·ChatGPTInterface·AdvancedFeaturesPanel·NotebookLLM CSS 로드, `brainwave-unified`·`data-brainwave-figma` |
| **대화** | `src/components/ChatGPTInterface.tsx` | `data-brainwave-figma`(디자인 링크), `brainwave-layout`, 사이드바·메인·환영·입력·모달. 입력 placeholder "Type '/' for commands", 캡빌리티 칩 colored background, 사이드바·모달 rgba → theme 토큰(`--sidebar-dark-*`, `--modal-overlay`, `--shadow-*`) 전환 완료 |
| **대화 스타일** | `src/components/ChatGPTInterface.css` | CORBU.AI 클래스, 입력 포커스 파란 링, 전송 호버, `.brainwave-chat-sidebar-dark`(Figma Chat list 다크), `.brainwave-chat-header`(제목·Share·액션 버튼), `.main-content`(bg·transition), 입력창·+ 버튼 가시성(.bw-composer .message-input·.input-icon-btn) |
| **대화형 레이아웃 (Figma)** | `ChatGPTInterface.css` `.message`·`.user-message`·`.assistant-message` | Figma Brainwave 대화형: 답변(AI) 왼쪽(`justify-content: flex-start`), 사용자 질문 오른쪽(`row-reverse`·`flex-end`). 사용자 아바타·버블 `--accent-info-figma`·`--accent-info-figma-muted`, 버블 `--radius-input-figma`. 입력창에 올린 내용 그대로 사용자 쪽에 표시. node 7-3·323-168775 기준. |
| **우측 Chat history** | `src/App.css`, `ChatGPTInterface.tsx` | `.brainwave-right-sidebar`(체크박스·제목·설명·시간·+ New chat), 접기/펼치기 애니메이션(.collapsed), ProjectShareDialog 닫기(X) 가시성 |
...
| **Ultimate 대화** | `src/components/UltimateChatGPTInterface.tsx` | theme 변수·`.bw-*` 전반 (레이아웃·사이드바·버튼·카드·입력·메시지 버블·로딩·드래그 영역·진행바) |
...
| **AI 대화 인터페이스** | `src/components/Chat/ChatInterface.tsx` | theme 변수·`.bw-*` (헤더·프로젝트 컨텍스트·메시지·타이핑·첨부·입력·전송·드래그 영역) |
...
| **간단 대화** | `src/views/SimpleChatView.js` + `App.css` | `.main-content`, `.chat-container`, `.message`, `.analysis-panel`, `.send-btn`, `.quick-action-btn` 등 theme 변수 사용 (CORBU.AI 동일). 테스트: `src/views/SimpleChatView.test.js` |
| **프로젝트 설정 모달** | `ProjectEditModal.tsx` + `ProjectEditModal.css` | theme 변수·`.bw-*` 기반 모달 (Figma node 7-3). Overlay·패널·입력·버튼·태그·파일 목록 전부 Brainwave Kit 적용 |
| **파일 분석 대화** | `FileAnalysisChatSystem.tsx` + `FileAnalysisChatSystem.css` | theme 변수·`.fac-*`·`getFileTypeStyle` (themeColors) 적용 |
| **AI 문서 생성기** | `AIDocumentGenerator.tsx` + `AIDocumentGenerator.css` | theme 변수·`.aidg-*`·`getCategoryStyle`·`getDocumentStatusStyle`·`getPriorityStyle` 적용 |
| **고급 검색** | `Search/AdvancedSearch.tsx` + `AdvancedSearch.css` | theme 변수·`.adv-search-*`·`.bw-modal-*` 적용 |
| **프로젝트 목록** | `ProjectManagement/ProjectList.tsx` + `ProjectList.css` | theme 변수·`.pl-*`·`getPriorityStyle`·`getProjectStatusStyle` 적용 |
| **프로젝트 생성 모달** | `ProjectManagement/ProjectCreationModal.tsx` | theme 변수·`.bw-btn-primary`·`.bw-input`·`.bw-modal-*` 적용 |
| **프로젝트 만들기 모달 (단일 단계)** | `ProjectManagement/ProjectCreateModal.tsx` + `ProjectCreateModal.css` | theme·`.bw-*` 전용 (오버레이·패널·입력·카테고리 칩·메모리·안내·CTA). [PROJECT_PAGE_DESIGN_SPEC.md](docs/PROJECT_PAGE_DESIGN_SPEC.md) |
| **프로젝트 · 대화·탭·소스 패널** | `ChatGPTInterface.tsx` + `ChatGPTInterface.css` | `.bw-project-guidance-hint`, `.bw-project-tabs`/`.bw-project-tab`, `.project-sources-panel`·리스트 (Figma node 7-3) |
| **고급 설정** | `Settings/AdvancedSettings.tsx` + `AdvancedSettings.css` | theme 변수·`.as-*`·`.bw-*` 전환 (프로필·알림·보안·외관·고급·푸터·확인 모달) |
| **RBAC 시스템** | `UserManagement/RBACSystem.tsx` + `RBACSystem.css` | theme 변수·`.rbac-*`·`getUserStatusStyle`·`getRoleBadgeStyle`·`getAuditStatusStyle` 적용 |
| **AI 성능 모니터** | `Monitoring/AIPerformanceMonitor.tsx` + `AIPerformanceMonitor.css` | theme 변수·`.apm-*`·`getHealthStyle`·`getModelStatusStyle`·`getAlertTypeStyle` 적용 |
| **지침 관리** | `Guidelines/GuidelinesManager.tsx` + `GuidelinesManager.css` | theme 변수·`.glm-*`·`getGuidelineCategoryStyle`·`getPriorityStyle` 적용 |
| **실시간 협업** | `Collaboration/RealTimeCollaboration.tsx` + `RealTimeCollaboration.css` | theme 변수·`.rtc-*`·`getCollaboratorRoleStyle`·`getCollaboratorStatusColor` 적용 |
| **뉴스 검색** | `News/NewsSearch.tsx` | theme 변수·`.bw-*`·`getSentimentColor` 적용 (검색·탭·카드·분석) |
| **워크플로우 자동화** | `Automation/WorkflowAutomation.tsx` + `WorkflowAutomation.css` | theme 변수·`.wfa-*`·`getWorkflowStatusStyle`·`getSeverityBadgeStyle` 적용 |
| **AI 엔진 모니터** | `AI/AIEngineMonitor.tsx` + `AIEngineMonitor.css` | theme 변수·`.aem-*`·`getStatusColor`·`getSentimentColor` 적용 (개요·모델·분석·설정 탭 전체) |
| **디자인 일관성** | [DESIGN_CONSISTENCY_REPORT.md](DESIGN_CONSISTENCY_REPORT.md) | 입력 폼 `bw-input` 통합·세부 페이지 적용 점검 (2026-02-20) |
| **사이드바 메뉴 구성** | [SIDEBAR_MENU_SIZE_SPEC.md](SIDEBAR_MENU_SIZE_SPEC.md) | 프로젝트/대화 섹션 높이·스크롤·노출 개수·접힌 모드·theme 변수 `--sidebar-projects-max-height` |

---

## 전체 디자인 적용 원칙 (2025 갱신)

- **단일 소스**: Figma Brainwave AI UI Kit node 7-3·323-168775를 기준으로 **전체 디자인**을 그대로 적용.
- **모든 기능 통합**: 3분할 라우트가 동일한 Brainwave 레이아웃·사이드바·메인 영역 안에서 동작.
  - 대화 `/`: 일반 대화·TTS·음성·이미지·예측·파일 첨부 등 통합
  - 프로젝트 `/projects`: 목록·생성·관리
  - 프로젝트 · 대화 `/projects/:id`: 노트북 LLM·파일 분석·AI 문서 생성
  - 전체 기능 `/features-map`: 도구 메뉴에서 접근
- **기능 접근**: 사이드바 네비로 직접 이동 또는 **전체 기능** 화면에서 카테고리별 링크로 모든 기능 접근 가능.
- **토큰·클래스**: `theme.css` `var(--*)`, `brainwave-global.css` `.bw-*`만 사용. Tailwind 색상 클래스(`text-gray-*`, `bg-blue-*`) 금지.
...
| Figma | CSS 변수 | 용도 |
|-------|----------|------|
| Primary/01 | `--accent-info` | 보조 CTA (#0084FF) |
| **Figma Primary Blue** | `--accent-info-figma` | 메인 CTA·Share·New chat (#3478F6) |
...
| 레이아웃(사이드바 등) | `--layout-sidebar-width` (260px), `--layout-main-padding`, `--layout-input-dock-padding`, `--layout-main-content-padding` | Figma node 7-3 Dev Mode에서 수치 확인 후 `theme.css`에서만 조정 |
...
- **대화 레이아웃 다크**: `html.dark-mode`(ThemeProvider) 또는 `.brainwave-layout.dark`(로컬 테마) 시 Figma Neutral/05·06·07 사용.
...
- **통합 앱 라우트(`/`, `/projects`, `/projects/:id`, `/features-map`)에서 사용하는 화면은 모두 theme·`.bw-*` 기준으로 통일됨.
...
- **설정 완료**: Figma 플러그인(MCP) 인증 후 Brainwave UI Kit 접근 가능.
- **파일 키**: `9ZrEa3dcS8zb0O6Nr5lT8m`
- **주요 node-id**: `7-3`(레이아웃), `323-168775`(전체 UI). URL에 `?node-id=7-3` 추가 후 Cursor에서 "이 Figma 디자인 구현해줘" 등으로 요청.
- **규칙 파일**: `.cursor/rules/figma-design-system.mdc` — Figma 기반 구현 시 이 규칙이 자동 적용됨.
