...
.PHONY: setup start stop check plugins status help sync-frontend sync-frontend-chat-input sync-frontend-unified-chat check-frontend-parity test-sidebar-context test-composer-pipeline test-conversation-graph-unit verify-pre-deploy maintain-push-block refresh-handoff verify-handoff check-doc-verification-hub
...
help:
	...
	@echo "  make maintain-push-block - 원격 push 막힘 시 아티팩트·회귀·진단·리포트(npm run maintain:push-block; HANDOFF_REFRESH=1 시 bundle 재생성)"
	@echo "  make refresh-handoff / verify-handoff - bundle·patch 이관 아티팩트(npm run refresh:handoff-artifacts / verify:handoff-artifacts)"
